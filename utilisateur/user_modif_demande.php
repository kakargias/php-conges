...
<?php 
	...
	if(isset($_POST['new_debut'])) { $new_debut=fr_date_to_eng($_POST['new_debut']); }
	if(isset($_POST['new_demi_jour_deb'])) { $new_demi_jour_deb=$_POST['new_demi_jour_deb']; }
	if(isset($_POST['new_fin'])) { $new_fin=fr_date_to_eng($_POST['new_fin']); }
...
function modifier($p_num_to_update, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $onglet) {
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id() ;
