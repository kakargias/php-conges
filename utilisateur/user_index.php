...
//			if($config_rtt_comme_conges==TRUE)
	...
	if($onglet=="historique_conges")
	{
		...
		$sql2 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat 
				...
				AND (p_etat='ok' OR  p_etat='refusé' OR  p_etat='annulé') ";
		if($tri_date=="descendant")
		...
		else
		{
			...
			while ($resultat2 = mysql_fetch_array($ReqLog2)) {
					...
					echo "<tr align=\"center\">\n";
					echo "<td class=\"histo\">$sql_p_date_deb _ $demi_j_deb</td><td class=\"histo\">$sql_p_date_fin _ $demi_j_fin</td><td class=\"histo\">$sql_p_nb_jours</td><td class=\"histo\">$sql_p_commentaire</td><td class=\"histo\">$sql_p_type</td><td class=\"histo\">$sql_p_etat</td>\n" ;
					echo "</tr>\n";
			}
			printf("</table>\n\n");
		}
	...
	if($onglet=="historique_autres_absences")
	{
		echo "<td colspan=$nb_colonnes>\n";
		// Récupération des informations
		$sql4 = "SELECT p_login, p_date_deb, p_date_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_num 
		...
		else
		{
			...
			while ($resultat4 = mysql_fetch_array($ReqLog4))
			{
				...
				if(($sql_etat != "annulé")&&($config_user_saisie_mission==TRUE))
				{
				...
				echo "<td class=\"histo\">$sql_date_deb</td><td class=\"histo\">$sql_date_fin</td>
					<td class=\"histo\">$sql_nb_jours</td><td class=\"histo\">$sql_commentaire</td>
					<td class=\"histo\">$sql_type</td><td class=\"histo\">$sql_etat</td>
