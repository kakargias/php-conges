...
//	if(isset($_GET['p_num'])) { $p_num=$_GET['p_num']; }
	...
	if($ajout_conges=="TRUE") {
		ajout_conges($mysql_link, $tab_champ_saisie_conges, $tab_champ_saisie_rtt);
	}
	elseif($ajout_global=="TRUE") {
		ajout_global($new_nb_conges_all, $new_nb_rtt_all, $calcul_new_conges_proportionnel, $calcul_new_rtt_proportionnel, $mysql_link);
	}
	elseif($ajout_groupe=="TRUE") {
		ajout_global_groupe($choix_groupe, $new_nb_conges_all, $new_nb_rtt_all, $calcul_new_conges_proportionnel, $calcul_new_rtt_proportionnel, $mysql_link);
	}
	else
	{
		saisie($mysql_link);
...
function saisie($mysql_link) {
	...
	{
		$sql1 = $sql1." u_resp_login = '".$_SESSION['userlogin']."' ";
	...
	$ReqLog1 = mysql_query( $sql1, $mysql_link) or die("ERREUR : mysql_query : ".$sql." --> ".mysql_error());
...
//				$sql_group = "SELECT g_gid, g_groupename FROM conges_groupe, conges_groupe_resp WHERE g_gid=gr_gid AND gr_login='".$_SESSION['userlogin']."' ORDER BY g_groupename "  ;
				$sql_group = "SELECT g_gid, g_groupename FROM conges_groupe WHERE g_gid IN ($list_group) ORDER BY g_groupename "  ;
				$ReqLog_group = mysql_query($sql_group, $mysql_link) or die("ERREUR : mysql_query : $sql_group :\n".mysql_error());
...
function ajout_conges($mysql_link, $tab_champ_saisie_conges, $tab_champ_saisie_rtt) 
{
	$session=session_id(); 
	...
	while($elem_tableau = each($tab_champ_saisie_conges))
	{
		...
		if($user_nb_jours_ajout_float!=0)
		{
			...
			$ReqLog1 = mysql_query($sql1, $mysql_link) or die("ERREUR : mysql_query : $sql1 :\n".mysql_error());

			// on insert l'ajout de conges dans la table periode
			$commentaire="ajout jour";
			insert_ajout_dans_periode($user_name, $user_nb_jours_ajout_float, "conges", $commentaire, $mysql_link);
		}
	}
	...
	if($_SESSION['config']['rtt_comme_conges']==TRUE)
	{
		while($elem_tableau_rtt = each($tab_champ_saisie_rtt))
		{
			...
			if($user_nb_jours_ajout_float!=0)
			{
				...
				/* On valide l'UPDATE dans la table ! */
				$ReqLog2 = mysql_query($sql2, $mysql_link) or die("ERREUR : mysql_query : $sql2 :\n".mysql_error());
...
function ajout_global($new_nb_conges_all, $new_nb_rtt_all, $calcul_new_conges_proportionnel, $calcul_new_rtt_proportionnel, $mysql_link)
{
	$session=session_id() ;
	...
	if( ($new_nb_conges_all!=0)||($new_nb_rtt_all!=0) ) // s'il sont tous les 2 à 0, ont ne fait rien
	{
		...
		{
			$req_update = $req_update." WHERE  u_resp_login='".$_SESSION['userlogin']."' " ;
			if($_SESSION['config']['gestion_groupes'] == TRUE)
			{
				$list_users_group=get_list_users_des_groupes_du_resp($_SESSION['userlogin'], $mysql_link);
				if($list_users_group!="")
		...
		//echo "$req_update<br>\n";
		/* On valide l'UPDATE dans la table ! */
		$ReqLog1 = mysql_query($req_update, $mysql_link) or die("ERREUR : mysql_query : ".$req_update." --> ".mysql_error());

		// on insert l'ajout de conges GLOBAL (pour tous les users) dans la table periode
		$commentaire="ajout pour tous les personnels";
		insert_ajout_global_dans_periode($new_nb_conges_all, $new_nb_rtt_all, $commentaire, $calcul_new_conges_proportionnel, $calcul_new_rtt_proportionnel,$mysql_link);
	}
...
function ajout_global_groupe($choix_groupe, $new_nb_conges_all, $new_nb_rtt_all, $calcul_new_conges_proportionnel, $calcul_new_rtt_proportionnel, $mysql_link)
{
	$session=session_id() ;
	...
	if( ($new_nb_conges_all!=0)||($new_nb_rtt_all!=0) ) // s'il sont tous les 2 à 0, ont ne fait rien
	{
		...
		$ReqLog_name = mysql_query($req_name, $mysql_link) or die("ERREUR : ajout_global_groupe() : $req_name :\n".mysql_error());
		...
		$ReqLog_g = mysql_query($req_g, $mysql_link) or die("ERREUR : ajout_global_groupe() : $req_g :\n".mysql_error());
		while ($resultat_g = mysql_fetch_array($ReqLog_g)) 
		{
			...
			//echo "$req_update<br>\n";
			/* On valide l'UPDATE dans la table ! */
			$ReqLog1 = mysql_query($req_update, $mysql_link) or die("ERREUR : ajout_global_groupe() : $req_update :\n".mysql_error());
			...
			if($new_nb_conges_all!=0)
			{
				...
				insert_ajout_dans_periode($current_login, $nb_conges, "conges", $commentaire, $mysql_link);
			}

			if( ($_SESSION['config']['rtt_comme_conges']==TRUE) && ($new_nb_rtt_all!=0) )
			{
				// ajout rtt
				if($calcul_new_rtt_proportionnel!=TRUE)
				...
				insert_ajout_dans_periode($current_login, $nb_rtt, "rtt", $commentaire, $mysql_link);
			}
		}
	}
...
function insert_ajout_global_dans_periode($nb_jours_conges, $nb_jours_rtt, $commentaire, $calcul_conges_proportionnel, $calcul_rtt_proportionnel, $mysql_link)
{
	...
	if($_SESSION['config']['responsable_virtuel']==FALSE)
	{
		$req_select = $req_select." WHERE  u_resp_login='".$_SESSION['userlogin']."' " ;
		if($_SESSION['config']['gestion_groupes'] == TRUE)
		{
			$list_users_group=get_list_users_des_groupes_du_resp($_SESSION['userlogin'], $mysql_link);
			if($list_users_group!="")
	...
	while ($resultat = mysql_fetch_array($ReqLog2)) 
	{
		...
		// ajout rtt
		if( ($_SESSION['config']['rtt_comme_conges']==TRUE) && ($nb_jours_rtt!=0) )
		{
