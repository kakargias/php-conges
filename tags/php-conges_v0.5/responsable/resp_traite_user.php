...
function affichage($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin) {
	...
	if($config_user_saisie_demande==1) {
		// Récupération des informations
		$sql2 = "SELECT p_login, p_date_deb, p_date_fin, p_nb_jours, p_commentaire, p_etat, p_num FROM conges_periode WHERE p_login = '$user_login' ANd ( p_etat ='demande' OR p_etat ='demande_rtt' ) ORDER BY p_date_deb";
	...
	/*******************/
	// Récupération des informations
	$sql3 = "SELECT p_login, p_date_deb, p_date_fin, p_nb_jours, p_commentaire, p_etat, p_num FROM conges_periode WHERE p_login = '$user_login' AND ( p_etat !='demande' OR p_etat !='demande_rtt' )  ORDER BY p_date_deb";
	...
	else
	{
		...
		while ($resultat3 = mysql_fetch_array($ReqLog3)) {
				$etat=$resultat3[5];
				if( (strstr($etat, "annulé")==FALSE) && (strstr($etat, "refusé")==FALSE) ) {
...
function annule_conges($user_login) {
	...
	while($elem_tableau = each($tab_checkbox_annule))
	{
		...
		elseif($user_etat=="rtt_prise")
			$sql1 = "UPDATE conges_periode SET p_etat=\"rtt_annulée\" WHERE p_num=$numero_int" ;
		else
			$sql1 = "UPDATE conges_periode SET p_etat=\"absence-annulée\" WHERE p_num=$numero_int" ;
		
		//echo($sql1."<br>");
		$ReqLog1 = mysql_query($sql1, $link) or die("ERREUR : mysql_query : ".$sql1." --> ".mysql_error());

		/* UPDATE table "conges_users" (jours restants) */
		if($user_etat=="pris") // on re-crédite les jours seulement pour des conges pris (pas pour les absences autres)
		{
...
//		$link = connexion_mysql() ;
		...
		echo "$user_login---$new_debut---$new_fin---$new_nb_jours---$new_comment---$new_etat---$num_new_demande<br>\n";
