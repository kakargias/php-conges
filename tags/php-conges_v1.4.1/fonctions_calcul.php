...
function compter($user, $date_debut, $date_fin, $opt_debut, $opt_fin, &$comment, $mysql_link, $DEBUG=FALSE)
{
	...
	if( ($date_debut!=0) && ($date_fin!=0) )
	{
		...
		/************************************************************/
		// 2 : on verifie que le conges demandé ne chevauche pas une periode deja posée
		if(verif_periode_chevauche_periode_user($date_debut, $date_fin, $user, $tab_periode_calcul, $comment, $mysql_link, $DEBUG) == TRUE)
			return 0;
...
function verif_periode_chevauche_periode_groupe($date_debut, $date_fin, $tab_periode_calcul, $groupe_id, $mysql_link, $DEBUG=FALSE)
{
	/*****************************/
	...
	foreach($tab_users as $current_login)
	{
		...
		$comment="";
		if(verif_periode_chevauche_periode_user($date_debut, $date_fin, $current_login, $tab_periode_calcul, $comment, $mysql_link, $DEBUG)==TRUE)
			return TRUE;
	}
}
...
// retourne TRUE si chevauchement et FALSE sinon !
function verif_periode_chevauche_periode_user($date_debut, $date_fin, $user, $tab_periode_calcul, &$comment, $mysql_link, $DEBUG=FALSE)
{
		/************************************************************/
		// 2 : on verifie que le conges demandé ne chevauche pas une periode deja posée
