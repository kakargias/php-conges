...
//			get_tab_resp_du_user($login_expediteur, $tab_resp, $mysql_link, $DEBUG);
		...
		else   // c'est un message du responsale à un user
		{
			...
			/****************************/
			if($objet=="valid_conges")  // c'est un mail de première validation de demande : il faut faire une copie au grand responsable
			{
				...
				if(count($tab_grd_resp)!=0)  // si tableau n'est pas vide
				{
					foreach($tab_grd_resp as $item_login)
					{
						...
						if($mail_dest_addr=="")
							...
							constuct_and_send_mail($objet, $mail_sender_name, $mail_sender_addr, $mail_dest_name, $mail_dest_addr, $num_periode, $mysql_link, $DEBUG=FALSE);
					}
				}
			}
		}
