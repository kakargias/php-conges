...
<?php
	...
	affichage_calendrier($year, $mois, $first_jour, $jour_today, $printable, $page_initiale, $selected);
	...
	if($printable!=1)   // si version ecran :
	{
		...
		affichage_boutons_defilement($first_jour, $mois, $year, $page_initiale) ;
...
/* Boutons de defilement */
/******************************/
function affichage_boutons_defilement($first_jour, $mois, $year, $page_initiale) 
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
...
// AFFICHAGE  TABLEAU (CALENDRIER)
function affichage_calendrier($year, $mois, $first_jour, $jour_today, $printable, $page_initiale, $selected)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
		...
		if($_SESSION['userlogin']=="conges") // si $_SESSION['userlogin'] est le responsable virtuel alors: affiche tous les users
		{
		...
		$ReqLog = mysql_query($sql, $mysql_link) or die("ERREUR : calendrier.php : ".mysql_error());
		...
		$tab_calendrier=recup_tableau_periodes($mois, $first_jour, $year, $mysql_link);
		...
		while ($resultat = mysql_fetch_array($ReqLog)) 
		{
			...
			if($printable==1)
				printf("<tr align=\"center\" class=\"cal-ligne-user-edit\">\n");
			...
			for($j=$first_jour; checkdate($mois, $j, $year); $j++) 
			{
				...
				affiche_cellule_jour_user($sql_login, $j_timestamp, $year, $mois_select, $j, $td_second_class, $printable, $tab_calendrier, $tab_rtt_echange, $tab_rtt_planifiees, $mysql_link);
...
// affichage de la cellule correspondant au jour et au user considéré 
function affiche_cellule_jour_user($sql_login, $j_timestamp, $year_select, $mois_select, $j, $second_class, $printable, $tab_calendrier, $tab_rtt_echange, $tab_rtt_planifiees, $mysql_link)
{
	$session=session_id();
	...
	recup_infos_artt_du_jour_from_tab($sql_login, $j_timestamp, $val_matin, $val_aprem, $tab_rtt_echange, $tab_rtt_planifiees, $mysql_link);
				...
				for ($i = 0; $i < $nb_resultat_periode; $i++) 
				{
					...
					if(in_array($sql_login, $tab_per))   // si la periode corespond au user que l'on est en train de traiter
					{
						...
						if( ($sql_p_date_deb==$date_j) && ($sql_p_date_fin==$date_j) )
						{
							...
							{
								$class_am=get_class_titre($sql_p_type, $sql_p_etat)."_am";
								$text_am="a";
							}
							if($sql_p_demi_jour_fin=="pm")
							{
								$class_pm=get_class_titre($sql_p_type, $sql_p_etat)."_pm";
								$text_pm="a";
							}
						}
						else
...
//	echo "      <table cellpadding=\"1\" cellspacing=\"1\" border=\"1\">\n" ;
	...
	echo "         <td class=\"cal-legende\"> week-end ou jour férié</td>\n" ;
	echo "      </tr>\n" ;
	echo "      <tr align=\"center\">\n" ;
...
function recup_tableau_periodes($mois, $first_jour, $year, $mysql_link)
{
	...
	for($j=$first_jour; checkdate($mois, $j, $year); $j++) 
	{
		...
		$user_periode_sql = "SELECT  p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_type, p_etat 
						FROM conges_periode 
						WHERE ( p_etat='ok' OR  p_etat='demande') AND (p_date_deb<='$date_j' AND p_date_fin>='$date_j') 
						ORDER BY p_date_deb ";
