...
if($_SESSION['config']['where_to_find_user_email']=="ldap"){ include("../config_ldap.php");}
	...
	$feuille_style=$_SESSION['config']['stylesheet_file']; 
	echo "<link href=\"../$feuille_style\" rel=\"stylesheet\" type=\"text/css\">\n";
	...
	if($_SESSION['config']['editions_papier']==TRUE)
	{
		...
		echo "<a href=\"../edition/edit_user.php?session=$session&user_login=".$_SESSION['userlogin']."\" target=\"_blank\"><img src=\"../img/edition-22x22.png\" width=\"22\" height=\"22\" border=\"0\" title=\"Editions Papier\" alt=\"Editions Papier\"></a> Editions Papier\n";
		echo "</td>\n";
	}
	...
	if(is_admin($_SESSION['userlogin'], $mysql_link))
	{
	...
	if(($new_demande_conges==1) && ($_SESSION['config']['user_saisie_demande']==TRUE)) {
		new_demande($new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $mysql_link);
	}
	elseif(($new_echange_rtt==1)&&($_SESSION['config']['user_echange_rtt']==TRUE)) {
		echange_absence_rtt($onglet, $new_debut, $new_fin, $new_comment, $moment_absence_ordinaire, $moment_absence_souhaitee, $mysql_link);
	}
	elseif($change_passwd==1) {
		change_passwd($mysql_link, $new_passwd1, $new_passwd2);
	}
	else {
		if($onglet=="")
			$onglet="historique_conges";
		affichage($onglet, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link);
	}
...
function affichage($onglet, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
	...
	$sql1 = "SELECT u_nom, u_prenom, u_nb_jours_an, u_solde_jours, u_nb_rtt_an, u_solde_rtt, u_quotite FROM conges_users where u_login = '".$_SESSION['userlogin']."' ";
	$ReqLog1 = mysql_query($sql1, $mysql_link) or die("ERREUR : user_index.php : ".mysql_error());
...
//	echo "<table cellpadding=\"0\" cellspacing=\"0\" border=\"1\" width=\"100%\">\n" ;
	...
	if($onglet=="nouvelle_absence")
	{
		...
		saisie_nouveau_conges($_SESSION['userlogin'], $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link);
	...
	if($onglet=="echange_jour_absence")
	{
		...
		//affiche le formulaire de saisie d'une nouvelle demande de conges
		saisie_echange_rtt($_SESSION['userlogin'], $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link);
	...
	if($onglet=="demandes_en_cours")
	{
		...
		$sql3 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_num 
				FROM conges_periode
				WHERE p_login = '".$_SESSION['userlogin']."' 
				AND (p_etat = 'demande') ";
	...
	if($onglet=="historique_autres_absences")
	{
		...
		$sql4 = "SELECT p_login, p_date_deb, p_date_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_motif_refus, p_num 
				FROM conges_periode WHERE p_login = '".$_SESSION['userlogin']."' 
		...
		$ReqLog4 = mysql_query($sql4, $mysql_link) or die("ERREUR : mysql_query : ".$sql4." --> ".mysql_error());
...
function new_demande($new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $mysql_link)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
	...
	if($valid==TRUE)
	{
		...
		$result=insert_dans_periode($_SESSION['userlogin'], $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $new_etat, $mysql_link);
		...
		if($result==TRUE)
		{
			...
			if($_SESSION['config']['mail_new_demande_alerte_resp']==TRUE)
				alerte_mail($_SESSION['userlogin'], ":responsable:", "new_demande");
		}
		else
...
function echange_absence_rtt($onglet, $new_debut, $new_fin, $new_comment, $moment_absence_ordinaire, $moment_absence_souhaitee, $mysql_link) 
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
	...
	else
	{
		...
		recup_infos_artt_du_jour($_SESSION['userlogin'], $j_timestamp_1, $val_matin, $val_aprem, $mysql_link);
		...
		recup_infos_artt_du_jour($_SESSION['userlogin'], $j_timestamp_2, $val_matin, $val_aprem, $mysql_link);
	...
	if($valid==TRUE)
	{
		...
		$result_verif_echange1 = mysql_query($sql_verif_echange1, $mysql_link) or die("ERREUR : echange_absence_rtt() :<br>\n".$sql_verif_echange1."<br>\n".mysql_error());
		...
		$result1 = mysql_query($sql1, $mysql_link) or die("ERREUR : echange_absence_rtt() :<br>\n".$sql1."<br>\n".mysql_error());
		...
		$result_verif_echange2 = mysql_query($sql_verif_echange2, $mysql_link) or die("ERREUR : echange_absence_rtt() :<br>\n".$sql_verif_echangeé."<br>\n".mysql_error());
		...
		$result2 = mysql_query($sql2, $mysql_link) or die("ERREUR : echange_absence_rtt() :<br>\n".$sql2."<br>\n".mysql_error());
...
function change_passwd($mysql_link, $new_passwd1, $new_passwd2) 
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
