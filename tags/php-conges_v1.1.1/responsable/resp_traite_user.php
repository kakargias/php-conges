...
function affichage($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link, $DEBUG)
{
	...
	echo "<H3>".$_SESSION['lang']['resp_traite_user_titre']."</H3><H2> $sql_prenom $slq_nom.</H2>\n\n";
...
//affiche l'état des demande du user (avec le formulaire pour le responsable)
function affiche_etat_demande_user_for_resp($user_login, $mysql_link, $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF']; ;
	$session=session_id() ;
	...
	else
	{
		...
		echo "<td class=\"titre\">".$_SESSION['lang']['resp_traite_user_motif_refus']."</td></tr>\n";
		$tab_checkbox=array();
