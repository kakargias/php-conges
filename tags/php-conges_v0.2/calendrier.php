...
global $config_temps_partiel_bgcolor, $config_demande_conges_bgcolor;

		//connexion mysql
		$link = connexion_mysql() ;

		// Récupération des users
		$sql = "SELECT u_login, u_nom, u_prenom, u_nb_jours_reste, u_quotite FROM conges_users ORDER BY u_nom, u_prenom";
...
global $link;
global $config_semaine_bgcolor, $config_temps_partiel_bgcolor, $config_conges_bgcolor, $config_demande_conges_bgcolor ;
	...
	else
	{
		...
		if(($session_username==$sql_login)&&($printable!=1))  // on affichera les demandes du user dans le calendrier seulement pour la version non iprimable, et pas pour les autres users
			$user_periode_sql = "SELECT  p_date_deb, p_date_fin, p_etat FROM conges_periode WHERE p_login='$sql_login' AND ( p_etat='pris' OR  p_etat='demande' ) AND (p_date_deb<='".$year_select."-".$mois_select."-".$j."' AND p_date_fin>='".$year_select."-".$mois_select."-".$j."') ";
		else
			$user_periode_sql = "SELECT  p_date_deb, p_date_fin, p_etat FROM conges_periode WHERE p_login='$sql_login' AND p_etat='pris' AND (p_date_deb<='".$year_select."-".$mois_select."-".$j."' AND p_date_fin>='".$year_select."-".$mois_select."-".$j."') ";
...
global $config_demande_conges_bgcolor, $config_temps_partiel_bgcolor;
	...
	echo "         <td> congès pris ou a prendre</td>\n" ;
	echo "      </tr>\n" ;
	echo "      <tr align=\"center\">\n" ;
	echo "         <td bgcolor=\"$config_demande_conges_bgcolor\">abs</td>\n" ;
	echo "         <td>congès demandé (non encore accordé)</td>\n" ;
	echo "      </tr>\n" ;
	echo "      <tr align=\"center\">\n" ;
	...
	echo "      <tr align=\"center\">\n" ;
	echo "         <td bgcolor=\"$config_temps_partiel_bgcolor\">-/abs</td>\n" ;
	echo "         <td>absapres midi pour temps partiel ou ARTT</td>\n" ;
	echo "      </tr>\n" ;
	echo "      </table>\n" ;
}
