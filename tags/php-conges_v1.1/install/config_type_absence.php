...
// REMARQUE :
// Dans cette page, on gere la possibilité de ne pas être dans une session (la page peut être appelée directement, 
// pour la première configuration de l'appli)
...
function affichage($tab_new_values, $mysql_link, $session, $DEBUG=FALSE)
{
	...
	/**************************************/
	// affichage du titre
	echo "<br><center><H1> Configuration des types d'absence gérées par PHP_CONGES</H1></center>\n";
	echo "<br>\n";
	/**************************************/
	// affichage de la liste des type d'absence existants
	
	//requête qui récupère les informations de la table conges_type_absence
	$sql1 = "SELECT * FROM conges_type_absence ORDER BY ta_type ASC";
	...
	while ($data = mysql_fetch_array($ReqLog1)) 
	{
		...
		if($old_type != $ta_type)
		{
			...
			{
				$legend="Congés" ;
				$comment="Les types d'absences listés ici sont des congés divers, décomptés chacuns sur des comptes séparés." ;
			}
			else
			{
				$legend="Absences" ;
				$comment="Les types d'absences listés ici ne sont pas décomptés (ce sont des absences autorisées)." ;
			}
			echo "<br>\n";
			...
			echo "    <tr><td class=\"titre\"><b><u>libellé</b></u></td><td class=\"titre\"><b><u>libellé court</b></u></td><td></td><td></td></tr>\n";
		}
				
		$text_modif="<a href=\"$URL?action=modif&id_to_update=$ta_id\">modifier</a>";
		$text_suppr="<a href=\"$URL?action=suppr&id_to_update=$ta_id\">supprimer</a>";
	...
	echo "    <fieldset class=\"cal_saisie\">\n";
	echo "    <legend class=\"boxlogin\">ajouter un type d'absence :</legend>\n";
	echo "    Saisissez le type d'absence que vous voulez ajouter :\n";	
	echo "    <table cellpadding=\"2\" >\n";
	echo "    <tr><td class=\"titre\">libellé</td><td class=\"titre\">libellé court</td><td class=\"titre\">type</td></tr>\n";	
	echo "    <tr>\n";	
	...
	$new_type = ( isset($tab_new_values['type']) ? $tab_new_values['type'] : 0 ) ;
	...
	echo "    <td class=\"histo\">\n";
	affiche_select_from_mysql_enum_field("conges_type_absence", "ta_type", "tab_new_values[type]", $new_type, "") ;
	echo "    </td></tr>\n";
	echo "    </table>\n";
	...
	echo "<br><br>\n";
	echo "<form>\n";
	echo "<center><input type=\"button\" value=\"Fermer cette Fenêtre\" onClick=\"javascript:window.close();\"></center>\n";
	echo "</form>\n";
	echo "<br><br>\n";
...
function modifier(&$tab_new_values, $mysql_link, $session, $id_to_update, $DEBUG=FALSE)
{
	...
	// affichage
	echo "<table cellpadding=\"2\" class=\"tablo\" >\n";
	echo "    <tr><td class=\"titre\"><b><u>libellé</b></u></td><td class=\"titre\"><b><u>libellé court</b></u></td><td class=\"titre\">type</td></tr>\n";
...
function commit_modif(&$tab_new_values, $mysql_link, $session, $id_to_update, $DEBUG=FALSE)
{
	...
	{
		echo "<br> saisie incorrecte :  les caractères suivants sont interdits:  \" \' , . ; % ? <br>\n";
		$erreur=TRUE;
	}
	// verif si les champs sont vides
	if( (strlen($tab_new_values['libelle'])==0) || (strlen($tab_new_values['short_libelle'])==0) )
	{
		echo "<br> saisie incorrecte :  des champs sont vides ! <br>\n";
		$erreur=TRUE;
	}
	...
	else
	{
		...
		echo "<span class = \"messages\">Modifications enregistrées avec succés !</span><br>";

		if($DEBUG==TRUE)
			echo "<a href=\"$URL\" method=\"POST\">Retour</a><br>\n" ;
		else
...
function supprimer($mysql_link, $session, $id_to_update, $DEBUG=FALSE)
{
	...
	{
		echo "<center>\n";
		echo "<br> Suppression IMPOSSIBLE !<br>\nDes congés/absences de ce type sont en cours ! <br>\n";
	...
	else
	{
		...
		echo "<center>\n";
		echo "<br>\n";
		echo "Veuillez confirmer la supression de <b>\" ".$tab_type_abs[$id_to_update]['libelle']." \"</b>\n";
		echo "<br>\n";
		echo "<form action=\"$URL\" method=\"POST\"> \n";
