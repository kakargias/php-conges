...
echo "<html>\n";
	echo "<head>\n";
		...
		echo '<script type="text/javascript" src="'. TEMPLATE_PATH .'jquery/js/jquery.ui.datepicker-fr.js"></script>';
		echo '<script type="text/javascript" src="'. TEMPLATE_PATH .'jquery/js/DataTables-1.9.0/media/js/jquery.dataTables.js"></script>';
	...
	*/
	...
	@import "'. TEMPLATE_PATH .'jquery/js/DataTables-1.9.0/media/css/demo_table_jui.css";
	...
	echo '<script>
		$(document).ready(function() { 
			...
			$("#dmd_in,#histo,#histo_abs").dataTable({
			"bJQueryUI": true,
			"bPaginate": false,
			"bLengthChange": false,
			"bFilter": false,
			...
			$( "input:submit,input:radio,input:button,input:reset, button", ".nouvelle_absence" ).button();
	...
	echo '<div id="header" class="nothing">' ;
			...
			// DEBUT AFFICHAGE DES BOUTONS ...
				...
				echo '<div id="dock"><div class="left"></div><ul>';
					...
					function bouton($name, $icon ,$link, $active = false)
					{
						...
						echo '<li class="'.($active?'selected':'').'"><a href="'. $link .'"><span>'.$name.'</span></a></li>';
					}
					...
					function bouton_popup($name, $icon ,$link, $popup_name, $size_x, $size_y, $active = false)
					{
						...
						echo '<li><a href="javascript:void(0);" onClick="javascript:OpenPopUp(\''. $link .'\',\''.$popup_name.'\','.$size_x.','.$size_y.');"><span>'.$name.'</span></a></li>'; 
					}
