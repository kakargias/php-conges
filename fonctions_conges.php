...
function alerte_mail($login_expediteur, $destinataire, $num_periode, $objet, $mysql_link, $DEBUG=FALSE)
{
//$DEBUG=TRUE;
...
//			get_tab_resp_du_user($login_expediteur, $tab_resp, $mysql_link, $DEBUG);
			...
			foreach($tab_resp as $item_login => $item_presence)
			{
				...
				else
				{
					...
					constuct_and_send_mail($new_objet, $mail_sender_name, $mail_sender_addr, $mail_dest_name, $mail_dest_addr, $num_periode, $mysql_link, $DEBUG);
				}
			}
		...
		else   // c'est un message du responsale à un user
		{
			...
			if($mail_dest_addr=="")
				...
				constuct_and_send_mail($objet, $mail_sender_name, $mail_sender_addr, $mail_dest_name, $mail_dest_addr, $num_periode, $mysql_link, $DEBUG);
...
function constuct_and_send_mail($objet, $mail_sender_name, $mail_sender_addr, $mail_dest_name, $mail_dest_addr, $num_periode, $mysql_link, $DEBUG=FALSE)
{
	if( $DEBUG==TRUE ) {echo "constuct_and_send_mail($objet, $mail_sender_name, $mail_sender_addr, $mail_dest_name, $mail_dest_addr, $num_periode)<br>\n";}
		...
		$res_abs = requete_mysql($select_abs, $mysql_link, "alerte_mail", $DEBUG);
...
function get_nb_users_du_groupe($group_id, $mysql_link, $DEBUG=FALSE)
{
   ...
   $nb_users = mysql_num_rows($ReqLog1);
...
function init_config_tab($DEBUG=FALSE)
{
	...
	$req_appli = mysql_query($sql_appli) or die ("ERREUR : mysql_query --> $sql_appli ".mysql_error($mysql_link));

	while ($data_appli = mysql_fetch_array($req_appli))
	{
		$key=$data_appli[0];
		$value=$data_appli[1];
	...
	$req_config = mysql_query($sql_config) or die ("ERREUR : mysql_query --> $sql_config ".mysql_error($mysql_link));

	while ($data = mysql_fetch_array($req_config))
	{
		$key=$data[0];
		$value=$data[1];
		$type=$data[2];
...
function recup_tableau_conges_for_user($login, $mysql_link, $DEBUG=FALSE)
{
	...
	if ($_SESSION['config']['gestion_conges_exceptionnels']==TRUE) // on prend tout les types de conges
		$sql_bilan = "SELECT ta_libelle, su_nb_an, su_solde, su_reliquat FROM conges_solde_user, conges_type_absence WHERE conges_type_absence.ta_id = conges_solde_user.su_abs_id AND su_login = '$login' ORDER BY su_abs_id ASC";
	else // on prend tout les types de conges SAUF les conges exceptionnels
		$sql_bilan = "SELECT ta_libelle, su_nb_an, su_solde, su_reliquat FROM conges_solde_user, conges_type_absence WHERE conges_type_absence.ta_type != 'conges_exceptionnels' AND conges_type_absence.ta_id = conges_solde_user.su_abs_id AND su_login = '$login' ORDER BY su_abs_id ASC";
...
function recup_infos_du_user($login, $list_groups_double_valid, $mysql_link, $DEBUG=FALSE)
{
	...
	$sql = "SELECT u_login, u_nom, u_prenom, u_is_resp, u_resp_login, u_is_admin, u_see_all, u_passwd, u_quotite, u_email, u_num_exercice FROM conges_users " .
			"WHERE u_login='$login' ";
...
function recup_infos_all_users_du_groupe($group_id, $mysql_link, $DEBUG=FALSE)
{
	$tab=array();

	// recup de la liste de tous les users du groupe ...
	$list_all_users_du_groupe = get_list_users_du_groupe($group_id, $mysql_link, $DEBUG);
	...
	if(strlen($list_all_users_du_groupe)!=0)
	{
		...
		foreach($tab_users_du_groupe as $current_login)
		{
			...
			$tab[$current_login] = recup_infos_du_user($current_login, $list_groupes_double_validation, $mysql_link, $DEBUG);
		}
	}
...
function get_reliquat_user_conges($login, $type_abs, $mysql_link, $DEBUG=FALSE)
{
	$select_info="SELECT su_reliquat FROM conges_solde_user WHERE su_login='$login' AND su_abs_id='$type_abs' ";
	$ReqLog_info = requete_mysql($select_info, $mysql_link, "get_reliquat_user_conges", $DEBUG);
...
//  soustrait_solde_et_reliquat_user($user_login, $num_current_periode, $user_nb_jours_pris, $type_abs, $date_deb, $demi_jour_deb, $date_fin, $demi_jour_fin, $mysql_link, $DEBUG=FALSE)
...
*/			
function soustrait_solde_et_reliquat_user($user_login, $num_current_periode, $user_nb_jours_pris, $type_abs, $date_deb, $demi_jour_deb, $date_fin, $demi_jour_fin, $mysql_link, $DEBUG=FALSE)
{
	...
	if($_SESSION['config']['autorise_reliquats_exercice']==TRUE)
	{
		...
		$reliquat=get_reliquat_user_conges($user_login, $type_abs, $mysql_link, $DEBUG);
		...
		if($_SESSION['config']['jour_mois_limite_reliquats']!=0)
		{
			...
			else
			{
				...
				$comment ="" ; // attention conserver cette variable sinon l'appele à la fonction suivante plante !!!
//				$nb_reliquats_a_deduire = compter($user_login, $date_deb, $_SESSION['config']['date_limite_reliquats'], $demi_jour_deb, "pm",  $comment ,      $mysql_link, $DEBUG);
				$nb_reliquats_a_deduire = compter($user_login, $num_current_periode, $date_deb, $_SESSION['config']['date_limite_reliquats'], $demi_jour_deb, "pm",  $comment ,      $mysql_link, $DEBUG);
	...
	$ReqLog2 = requete_mysql($sql2,$mysql_link, "soustrait_solde_et_reliquat_user", $DEBUG) ;				
}
