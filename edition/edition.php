...
<?php
	...
	echo "<TITLE> Impression Etat Conges : $session_username</TITLE>\n";
...
function edition($login, $edit_id, $mysql_link)
{
	...
	global $config_rtt_comme_conges, $config_texte_bas_edition_papier, $config_texte_haut_edition_papier ;
	...
	echo "<td>$config_texte_haut_edition_papier<br><br></td>\n";
	echo "</tr>\n";
	echo "</table>\n";
	...
	echo "<td>\n";
		...
		else
		{
			...
			while ($resultat2 = mysql_fetch_array($ReqLog2)) {
					...
					echo "<td class=\"histo-edit\">$sql_p_etat</td>\n" ;
					if($sql_p_etat=="ok")
	...
	echo "<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"770\">\n" ;
	echo "<tr align=\"center\">\n";
	echo "<td><br>$config_texte_bas_edition_papier</td>\n";
	echo "</tr>\n";
	echo "</table>\n";
...
function enregistrement_edition($login, $mysql_link)
{
	...
	$sql_update = "UPDATE conges_periode SET p_edition_id=$new_edition_id 
			...
			AND (p_etat='ok' OR  p_etat='annulé' OR  p_etat='refusé' OR  p_etat='ajout') ";
