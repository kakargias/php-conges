...
function commit_update($u_login_to_update)
{
	...
	global $new_is_resp, $new_resp_login, $new_is_admin, $new_login;
	...
	echo "$u_login_to_update---$new_nom---$new_prenom---$new_quotite---$new_nb_j_an---$new_solde_jours---$new_rtt_an---$new_solde_rtt---$new_is_resp---$new_resp_login---$new_is_admin---$new_email---$new_login<br>\n";
	...
	$sql1 = "UPDATE conges_users  
		SET	u_nom='$new_nom', 
			u_prenom='$new_prenom',
			u_nb_jours_an='$new_nb_j_an', 
			u_solde_jours='$new_solde_jours', 
			u_nb_rtt_an='$new_rtt_an', 
			u_solde_rtt='$new_solde_rtt', 
			u_is_resp='$new_is_resp', 
			u_resp_login='$new_resp_login', 
			u_is_admin='$new_is_admin', 
			u_login='$new_login', 
			u_quotite='$new_quotite', 
			u_email='$new_email' 
		WHERE u_login='$u_login_to_update' " ;
	$result1 = mysql_query($sql1, $link) or die("ERREUR : commit_update() : $sql1 <br>\n".mysql_error());
	if($result1==FALSE)
		$result==FALSE;
	...
	else
	{
		...
		$result_grille = mysql_query($sql_grille, $link) or die("ERREUR : commit_update() : $sql_grille <br>\n".mysql_error());
		...
		if($count_grille==0) // si pas de grille modifiée aujourd'hui : on update la date de fin de la derniere grille
		{
			...
			//echo "sql2 = $sql2<br>\n";
			$result2 = mysql_query($sql2, $link) or die("ERREUR : commit_update() : $sql2 <br>\n".mysql_error());
			if($result2==FALSE)
				$result==FALSE;
		}
		else  // si une grille modifiée aujourd'hui : on delete cette grille
		{
			...
			$result_suppr_grille = mysql_query($sql_suppr_grille, $link) or die("ERREUR : commit_update() : $sql_suppr_grille <br>\n".mysql_error());
