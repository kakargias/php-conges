...
function affichage($user_login,  $year_affichage, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link, $DEBUG)
{
	...
	if(($_SESSION['config']['user_saisie_demande']==FALSE)||($_SESSION['config']['resp_saisie_mission']==TRUE)) 
	{
		...
		saisie_nouveau_conges($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link);
	...
	affiche_etat_conges_user_for_resp($user_login,  $year_affichage, $tri_date, $mysql_link, $DEBUG);
...
//affiche l'état des conges du user (avec le formulaire pour le responsable)
function affiche_etat_conges_user_for_resp($user_login, $year_affichage, $tri_date, $mysql_link, $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF']; ;
	$session=session_id() ;
...
function annule_conges($user_login, $tab_checkbox_annule, $tab_text_annul, $mysql_link, $DEBUG=FALSE) 
{
	...
	while($elem_tableau = each($tab_checkbox_annule))
	{
		...
		// Log de l'action
		log_action($numero_int,"annul", $user_login, "annulation conges $numero ($user_login) ($user_nb_jours_pris jours)", $mysql_link, $DEBUG);
		...
		if($_SESSION['config']['mail_annul_conges_alerte_user']==TRUE)
			alerte_mail($_SESSION['userlogin'], $user_login, $numero_int, "annul_conges", $mysql_link, $DEBUG);
	}
...
//		$value_traite=$champs[3];
		...
		if($reponse == "ACCEPTE") // acceptation definitive d'un conges
		{
			...
			// Log de l'action
			log_action($numero_int,"ok", $user_login, "traite demande $numero ($user_login) ($user_nb_jours_pris jours) : $value_traite", $mysql_link, $DEBUG);
			...
			{
//				soustrait_solde_user($user_login, $user_nb_jours_pris_float, $value_type_abs_id, $mysql_link, $DEBUG);
				soustrait_solde_et_reliquat_user($user_login, $user_nb_jours_pris_float, $value_type_abs_id, $date_deb, $demi_jour_deb, $date_fin, $demi_jour_fin, $mysql_link, $DEBUG);
			}
		...
		elseif($reponse == "VALID") // première validation dans le cas d'une double validation  
		{
			...
			// Log de l'action
			log_action($numero_int,"valid", $user_login, "traite demande $numero ($user_login) ($user_nb_jours_pris jours) : $value_traite", $mysql_link, $DEBUG);
		...
		elseif($reponse == "REFUSE") // refus d'un conges
		{
			...
			// Log de l'action
			log_action($numero_int,"refus", $user_login, "traite demande $numero ($user_login) ($user_nb_jours_pris jours) : $value_traite", $mysql_link, $DEBUG);
...
function new_conges($user_login, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type_id, $mysql_link, $DEBUG=FALSE)
{
	...
	if($valid==TRUE)
	{
		...
		/**********************************/
		$new_etat="ok";
		$result=insert_dans_periode($user_login, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type_id, $new_etat, 0,$mysql_link, $DEBUG);
...
//			soustrait_solde_user($user_login, $user_nb_jours_pris_float, $new_type_id, $mysql_link, $DEBUG);
			soustrait_solde_et_reliquat_user($user_login, $user_nb_jours_pris_float, $new_type_id, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin , $mysql_link, $DEBUG);
		}
