...
function affichage($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date)
{
	...
	if($config_user_saisie_demande==TRUE) 
	{
		...
		else
		{
			...
			printf("<tr align=\"center\"><td class=\"titre\">Debut</td><td class=\"titre\">Fin</td><td class=\"titre\">nb Jours Pris</td><td class=\"titre\">Commentaire</td><td class=\"titre\">Type</td><td class=\"titre\">Accepter</td><td class=\"titre\">Refuser</td></tr>\n");
			$tab_checkbox=array();
			while ($resultat2 = mysql_fetch_array($ReqLog2)) 
			{
				...
				printf("<td class=\"histo\">$sql_date_deb _ $demi_j_deb</td><td class=\"histo\">$sql_date_fin _ $demi_j_fin</td><td class=\"histo\">$sql_nb_jours</td><td class=\"histo\">$sql_commentaire</td><td class=\"histo\">$sql_type</td><td class=\"histo\">$casecocher1</td><td class=\"histo\">$casecocher2</td>\n");
				printf("</tr>\n");
			}
			printf("</table>\n\n");
	...
	/*******************/
	// Récupération des informations
	$sql3 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_num FROM conges_periode WHERE p_login = '$user_login' AND p_etat !='demande' ";
	...
	else
	{
		...
		while ($resultat3 = mysql_fetch_array($ReqLog3)) 
		{
				...
				if(($sql_etat=="annulé") || ($sql_etat=="refusé")) 
					$casecocher1="";
				else
					$casecocher1=sprintf("<input type=\"checkbox\" name=\"tab_checkbox_annule[$sql_num]\" value=\"$sql_login-$sql_nb_jours-$sql_type-ANNULE\">");
				echo "<tr align=\"center\">\n";
				echo "<td class=\"histo\">$sql_date_deb _ $demi_j_deb</td><td class=\"histo\">$sql_date_fin _ $demi_j_fin</td>\n";
				echo "<td class=\"histo\">$sql_nb_jours</td><td class=\"histo\">$sql_commentaire</td>\n";
				echo "<td class=\"histo\">$sql_type</td><td class=\"histo\">$sql_etat</td>\n";
...
function traite_demandes($user_login) 
{
	...
	while($elem_tableau = each($tab_radio_traite_demande))
	{
		...
		else 
		{
			...
			{
				$sql3 = "UPDATE conges_periode SET p_etat=\"refusé\" WHERE p_num=$numero_int" ;
