...
<?php
	...
	if(!isset($tab_bt_radio)) {
		...
		traite_demande($mysql_link, $tab_bt_radio, $tab_text_refus);
	}
...
function saisie($mysql_link) 
{
	...
	{
		$sql1=$sql1." AND conges_users.u_login=conges_periode.p_login AND ( u_resp_login = '".$_SESSION['userlogin']."' ";
	...
	else
	{
		...
		echo "<td class=\"titre\"><b>nom</b><br>prenom</td><td class=\"titre\">Quotité</td>" ;
		echo "<td class=\"titre\">Debut</td><td class=\"titre\">Fin</td><td class=\"titre\">Commentaire</td><td class=\"titre\">nb Jours<br>Pris</td><td class=\"titre\">Solde<br>congés</td>" ;
		if($_SESSION['config']['rtt_comme_conges']==TRUE)
			echo "<td class=\"titre\">Solde<br>rtt</td><td class=\"titre\">Type</td>" ;
		echo "<td class=\"titre\">Accepter</td><td class=\"titre\">Refuser</td><td class=\"titre\">Attente</td><td class=\"titre\">Motif<br>de refus</td>\n" ;
		echo "</tr>\n";
...
//			printf("<td class=\"histo\">%s</td><td class=\"histo\">%s</td><td class=\"histo\">%d%%</td>", $sql_u_nom, $sql_u_prenom, $sql_u_quotite);
			printf("<td class=\"histo\"><b>$sql_u_nom</b><br>$sql_u_prenom</td><td class=\"histo\">$sql_u_quotite%%</td>");
...
//				printf("<td class=\"histo\">%s</td>", affiche_decimal($sql_u_solde_rtt) );
			...
			printf("<td class=\"histo\">$boutonradio1</td><td class=\"histo\">$boutonradio2</td><td class=\"histo\">$boutonradio3</td><td class=\"histo\">$text_refus</td>\n");
...
function traite_demande($mysql_link, $tab_bt_radio, $tab_text_refus) 
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
