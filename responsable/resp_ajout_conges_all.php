	...
	if( $config_gestion_groupes==TRUE )
	{
		...
		if($list_group!="")  //si la liste n'est pas vide ( serait le cas si n'est responsable d'aucun groupe)
		{
			...
			echo "		<td class=\"big\">choix du groupe : </td>\n";
				...
				$sql_group = "SELECT gr_groupename FROM conges_groupe_resp WHERE gr_login='$session_username' ORDER BY gr_groupename "  ;
				$ReqLog_group = mysql_query($sql_group, $link) or die("ERREUR : mysql_query : ".$sql_group." --> ".mysql_error());
				while ($resultat_group = mysql_fetch_array($ReqLog_group)) {
					$current_group=$resultat_group["gr_groupename"];
					$text_choix_group=$text_choix_group."<option value=\"$current_group\" >".$current_group."</option>";
				}
...
{
	global $PHP_SELF, $link;
	global $config_rtt_comme_conges, $config_responsable_virtuel;
	global $session, $session_username ;
	...
	$req_g="SELECT gu_login FROM conges_groupe_users WHERE gu_groupename='$choix_groupe' ";
	$ReqLog_g = mysql_query($req_g, $link) or die("ERREUR : mysql_query : ".$req_g." --> ".mysql_error());
