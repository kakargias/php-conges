...
function alerte_mail($login_expediteur, $destinataire, $objet, $mysql_link, $DEBUG=FALSE)
{
...
function affiche_tableau_bilan_conges_user($login, $mysql_link, $DEBUG=FALSE)
{
	...
	foreach($tab_cong_user as $id => $val)
		echo "<td class=\"titre\">".$tab_type_cong[$id]."/ ".$_SESSION['lang']['divers_an_maj']."</td><td class=\"titre\">".$_SESSION['lang']['divers_solde_maj']." ".$tab_type_cong[$id]."</td>" ;
	echo "</tr>\n";
...
function recup_infos_all_users($mysql_link, $DEBUG=FALSE)
{
	...
	$sql = "SELECT u_login FROM conges_users " .
			"WHERE u_login!='conges' AND u_login!='admin' ORDER BY u_login";
...
function recup_infos_all_users_du_grand_resp($login, $mysql_link, $DEBUG=FALSE)
{
	...
	if($list_groups_double_valid!="")
	{
		...
		$sql_users = "SELECT DISTINCT(gu_login) FROM conges_groupe_users WHERE gu_gid IN ($list_groups_double_valid) ; ";
...
function affiche_bouton_retour($session, $DEBUG=FALSE)
{
	...
	if($session=="")  
		echo "<center><a href=\"".$_SESSION['config']['URL_ACCUEIL_CONGES']."/install/?lang=".$_SESSION['config']['lang']."\">".$_SESSION['lang']['form_retour']."</a></center>\n";
	else
	{
		echo "<form action=\"".$_SESSION['config']['URL_ACCUEIL_CONGES']."/install/\" method=\"POST\">\n";
		echo "<input type=\"hidden\" name=\"lang\" value=\"".$_SESSION['config']['lang']."\">\n";
