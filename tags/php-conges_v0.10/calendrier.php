...
<?php
	...
	if($printable!=1)  // si version écran :
		echo "<body text=\"#000000\" link=\"#000080\" vlink=\"#800080\" alink=\"#FF0000\" background=$config_bgimage>\n";
...
global $config_rtt_comme_conges, $config_gestion_groupes, $config_affiche_groupe_in_calendrier ;
		...
		if($session_username=="conges") // si $session_username est le responsable virtuel alors: affiche tous les users
		{
			$sql = "SELECT DISTINCT u_login, u_nom, u_prenom, u_solde_jours, u_solde_rtt, u_quotite FROM conges_users ";
			$sql = $sql." WHERE u_login!='conges' ORDER BY u_nom, u_prenom";
		}
		else
		{
			$sql = "SELECT DISTINCT u_login, u_nom, u_prenom, u_solde_jours, u_solde_rtt, u_quotite FROM conges_users ";
			...
			//si affichage par groupe  (on affiche les membres des groupes du $session_username)
			if( ($config_gestion_groupes==TRUE) && ($config_affiche_groupe_in_calendrier==TRUE) ) 
			{
				$sql = $sql." AND ( u_login = '$session_username' ";
				//recup de la liste des users des groupes dont le user est membre 
				$list_users=get_list_users_des_groupes_du_user($session_username, $link); 
				if($list_users!="")  //si la liste n'est pas vide ( serait le cas si n'est membre d'aucun groupe)
					$sql = $sql." OR u_login IN ($list_users) ";

				// si $session_username est responsable (on affiche en +, les membres des groupes dont $session_username est resp)
				if(is_resp($session_username, $link)==TRUE)
				{
				//recup de la liste des users des groupes dont le user est responsable
					$list_users_2=get_list_users_des_groupes_du_resp($session_username, $link); 
			...
			{
				// si $session_username est responsable
				if(is_resp($session_username, $link)==TRUE)
					$sql = $sql." AND ( u_login = '$session_username' OR u_resp_login = '$session_username' ) ";
			}
