...
function affiche_cellule_calendrier_echange_absence_saisie_semaine($val_matin, $val_aprem, $year, $mois, $j, $DEBUG=FALSE)
{
	...
	if( $val_matin == 'Y' && $val_aprem == 'Y')
		{echo '<td bgcolor='.$bgcolor.' class="cal-saisie"><label>'.$j.'</label><input type="radio" name="new_debut" value="'.$year.'-'.$mois.'-'.$j.'-j"></td>';
		 $array_rtt[]= $year.'-'.$mois.'-'.$j.'-j'; print_r($array_rtt);}
...
function test_date_rtt ($sql_login)
{
$date_j				= date('Y-m-d', $j_timestamp);
$sql_artt='SELECT *	FROM conges_artt WHERE	a_login	=	\''.SQL::quote($sql_login).'\'';
							// echo $sql_artt;
...
$ar_rtt = $result_artt;
// echo count($ar_rtt);
for ($i=0;$i<count($ar_rtt);$i++)
{
	// echo $ar_rtt[$i];
	if ($ar_rtt[$i]=='Y'){$array_artt[]= mysql_field_name($result,$i);}
}
return json_encode($array_artt);
}
...
function affiche_tableau_bilan_conges_user($login, $DEBUG=FALSE)
{
	...
	echo '<table id="table_solde" >';
		echo '<thead>';
			echo '<tr>';
				echo '<th>'. _('divers_quotite') .'</th>';
				foreach($tab_cong_user as $id => $val) {
					if ($_SESSION['config']['gestion_conges_exceptionnels'] && in_array($id,$tab_type_conges_exceptionnels))
						echo '<th >'. _('divers_solde_maj') .' '.$id.'</th>';
					else
						echo '<th>'.$id.' / '. _('divers_an_maj') .'</th><th>'. _('divers_solde_maj') .' '.$id.'</th>';
				}
			echo '</tr>';
		echo '</thead>';
		echo '<tbody>';
			echo '<tr>';
				...
				foreach($tab_cong_user as $id => $val){
					if ($_SESSION['config']['gestion_conges_exceptionnels']  && in_array($id,$tab_type_conges_exceptionnels))
						echo '<td>'.$val['solde'].( $val['reliquat'] > 0 ? ' ('. _('dont_reliquat').' '.$val['reliquat'].')': '') .'</td>';
					else
						echo '<td>'.$val['nb_an'].'</td><td>'.$val['solde'].( $val['reliquat'] > 0 ?' ('. _('dont_reliquat').' '.$val['reliquat'].')':'').'</td>';
				}
			echo '</tr>';
		echo '</tbody>';
	echo '</table>';
...
function affiche_select_from_lang_directory( $select_name, $default )
{
	...
	echo '<select id="'.$select_name.'" name="'.$select_name.'">';
...
/*JOUR FERIE PERSO*/
function init_tab_jours_off($user,  $DEBUG=FALSE)
{
...
//affiche le formulaire de saisie d'une nouvelle demande de conges NEW LOOK avec Datapicker
function saisie_nouveau_conges_datepicker($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet,  $DEBUG=FALSE) {
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
	...
	if ((d_debut) && (d_fin)){
		...
		$.get('../xhr_calcul_jour_pris.php', {
			...
			if (parseInt(data)>=0) {
					...
					document.getElementById('error_calcul').style.display= 'None';
					return true;
					}
					else{
					colorize('#ff8888');
					document.getElementById('new_nb_jours').value= '';
					document.getElementById('error_calcul').style.display= '';
					document.getElementById('error_calcul').innerHTML= '<p><span class="ui-icon ui-icon-alert" style="float: left; margin-right: .3em;"></span><strong>Alert: </strong><br/>'+ data+' </p>';
					$( "#error_calcul" ).show( 'slide',  500 );
					//alert(data);
					}
					}
			);
...
function reset_calendar() {
	...
	$( "#from, #to" ).datepicker( "refresh" ); /*don't work :(*/
}
</script>	
<?php	
echo '
<div class="content">
<div id="radio">
<table class="tabledatepick" border=2 cellspacing="20" cellpadding="10" style="border: medium solid " rules="none"  >
<tr>



<td>
<label for="new_debut">Du </label><input type="text" id="from" name="new_debut"/><br/>
<div class="radioline">
...
<label for="new_fin">jusqu\'au </label><input type="text" id="to" name="new_fin"/><br/>
<div class="radioline">
...
echo '<input type="hidden" id="login"  name="login_user" value="'.$user_login.'">';
					...
					// }
					// zones de texte
					echo '<span id="NB"><b>'. _('saisie_conges_nb_jours') .'</b></span><br/>';

					if($_SESSION['config']['disable_saise_champ_nb_jours_pris'])  // zone de texte en readonly et grisée
						$text_nb_jours ='<input type="text" id="new_nb_jours" style="background-color: white;text-align:center;" name="new_nb_jours" size="10" maxlength="30" value="" style="background-color: #D4D4D4; " readonly="readonly">' ;
					else
						$text_nb_jours ='<input type="text" style="background-color: white;text-align:center;" id="new_nb_jours" name="new_nb_jours" size="10" maxlength="30" value="">' ;

					$text_commentaire='<input type="text" name="new_comment" size="25" maxlength="30" value="">' ;
					//echo '<tr align="center">';
					echo ($text_nb_jours).'<br/>';
					echo '<span id="COM"><b>'. _('divers_comment_maj_1') .'</b></span><br/>'.$text_commentaire.'<br/>';
					// echo '</tr>';
					// echo '<tr align="center"><td><img src="'. TEMPLATE_PATH . 'img/shim.gif" width="15" height="10" border="0" vspace="0" hspace="0"></td><td></td></tr>';
					// echo '<tr align="center">';
					// echo '<td colspan=2>';
						echo '<input type="hidden" name="user_login" value="'.$user_login.'">';
						echo '<input type="hidden" name="new_demande_conges" value=1>';
						echo '<br/><div id="error_calcul" class="ui-state-error ui-corner-all" style="padding: 0 .7em;display:None;"><p></p></div>';
						// boutons du formulaire
						// les classes "button_type_submit" et "button_type_cancel"
						// servent à choisir leur position (droite gauche) dans vos feuilles de style (voir style.css)
						//echo '<input type="submit" class="button_type_submit" value="'. _('form_submit') .'">   <input type="reset" class="button_type_cancel" value="'. _('form_cancel') .'">';
...
echo '</tr>';
echo '<tr><td colspan="3" align="center"><input type="submit" class="button_type_submit" value="'. _('form_submit') .'">   <input type="reset" onclick="reset_calendar()" class="button_type_cancel" value="'. _('form_cancel') .'"></td></tr>';
echo '</table><br/>';
echo '<div></div>';
