...
//$DEBUG=TRUE;
	...
	{
		$valid_1=verif_saisie_decimal($tab_new_jours_an[$id_conges], $DEBUG);  //verif la bonne saisie du nombre décimal
		$valid_2=verif_saisie_decimal($tab_new_solde[$id_conges], $DEBUG);  //verif la bonne saisie du nombre décimal
	}
	if ($_SESSION['config']['gestion_conges_exceptionnels']==TRUE)
	{
		foreach($tab_type_conges_excep as $id_conges => $libelle)
		{
			$valid_3=verif_saisie_decimal($tab_new_solde[$id_conges], $DEBUG);  //verif la bonne saisie du nombre décimal
		}
	}
	else
		$valid_3=TRUE;
