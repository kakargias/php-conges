...
function e4_maj_table_conges_config( $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
		...
		if($config_auth) {$config_auth="TRUE";} else {$config_auth="FALSE";} ;
		...
		$result = SQL::query($sql1);
		if($config_export_users_from_ldap) {$config_export_users_from_ldap="TRUE";} else {$config_export_users_from_ldap="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_export_users_from_ldap' WHERE `conf_nom` = 'export_users_from_ldap' ;";
		$result = SQL::query($sql1);
		
		if($config_user_saisie_demande) {$config_user_saisie_demande="TRUE";} else {$config_user_saisie_demande="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_user_saisie_demande' WHERE `conf_nom` = 'user_saisie_demande' ;";
		$result = SQL::query($sql1);
		if($config_user_affiche_calendrier) {$config_user_affiche_calendrier="TRUE";} else {$config_user_affiche_calendrier="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_user_affiche_calendrier' WHERE `conf_nom` = 'user_affiche_calendrier' ;";
		$result = SQL::query($sql1);
		if($config_user_saisie_mission) {$config_user_saisie_mission="TRUE";} else {$config_user_saisie_mission="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_user_saisie_mission' WHERE `conf_nom` = 'user_saisie_mission' ;";
		$result = SQL::query($sql1);
		if($config_user_ch_passwd) {$config_user_ch_passwd="TRUE";} else {$config_user_ch_passwd="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_user_ch_passwd' WHERE `conf_nom` = 'user_ch_passwd' ;";
		$result = SQL::query($sql1);
		
		if($config_responsable_virtuel) {$config_responsable_virtuel="TRUE";} else {$config_responsable_virtuel="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_responsable_virtuel' WHERE `conf_nom` = 'responsable_virtuel' ;";
		$result = SQL::query($sql1);
		if($config_resp_affiche_calendrier) {$config_resp_affiche_calendrier="TRUE";} else {$config_resp_affiche_calendrier="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_resp_affiche_calendrier' WHERE `conf_nom` = 'resp_affiche_calendrier' ;";
		$result = SQL::query($sql1);
		if($config_resp_saisie_mission) {$config_resp_saisie_mission="TRUE";} else {$config_resp_saisie_mission="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_resp_saisie_mission' WHERE `conf_nom` = 'resp_saisie_mission' ;";
		$result = SQL::query($sql1);
		if($config_resp_vertical_menu) {$config_resp_vertical_menu="TRUE";} else {$config_resp_vertical_menu="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_resp_vertical_menu' WHERE `conf_nom` = 'resp_vertical_menu' ;";
		$result = SQL::query($sql1);
		
		if($config_admin_see_all) {$config_admin_see_all="TRUE";} else {$config_admin_see_all="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_admin_see_all' WHERE `conf_nom` = 'admin_see_all' ;";
		$result = SQL::query($sql1);
		if($config_admin_change_passwd) {$config_admin_change_passwd="TRUE";} else {$config_admin_change_passwd="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_admin_change_passwd' WHERE `conf_nom` = 'admin_change_passwd' ;";
		$result = SQL::query($sql1);
		
		if($config_mail_new_demande_alerte_resp) {$config_mail_new_demande_alerte_resp="TRUE";} else {$config_mail_new_demande_alerte_resp="FALSE";} ;
		...
		$result = SQL::query($sql1);
		if($config_mail_valid_conges_alerte_user) {$config_mail_valid_conges_alerte_user="TRUE";} else {$config_mail_valid_conges_alerte_user="FALSE";} ;
		...
		$result = SQL::query($sql1);
		if($config_mail_refus_conges_alerte_user) {$config_mail_refus_conges_alerte_user="TRUE";} else {$config_mail_refus_conges_alerte_user="FALSE";} ;
		...
		$result = SQL::query($sql1);
		if($config_mail_annul_conges_alerte_user) {$config_mail_annul_conges_alerte_user="TRUE";} else {$config_mail_annul_conges_alerte_user="FALSE";} ;
		...
		if($config_samedi_travail) {$config_samedi_travail="TRUE";} else {$config_samedi_travail="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_samedi_travail' WHERE `conf_nom` = 'samedi_travail' ;";
		$result = SQL::query($sql1);
		if($config_dimanche_travail) {$config_dimanche_travail="TRUE";} else {$config_dimanche_travail="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_dimanche_travail' WHERE `conf_nom` = 'dimanche_travail' ;";
		$result = SQL::query($sql1);
		
		if($config_gestion_groupes) {$config_gestion_groupes="TRUE";} else {$config_gestion_groupes="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_gestion_groupes' WHERE `conf_nom` = 'gestion_groupes' ;";
		$result = SQL::query($sql1);
		if($config_affiche_groupe_in_calendrier) {$config_affiche_groupe_in_calendrier="TRUE";} else {$config_affiche_groupe_in_calendrier="FALSE";} ;
		...
		if($config_editions_papier) {$config_editions_papier="TRUE";} else {$config_editions_papier="FALSE";} ;
		...
		if($config_rtt_comme_conges) {$config_rtt_comme_conges="TRUE";} else {$config_rtt_comme_conges="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_rtt_comme_conges' WHERE `conf_nom` = 'rtt_comme_conges' ;";
		$result = SQL::query($sql1);

		if($config_user_echange_rtt) {$config_user_echange_rtt="TRUE";} else {$config_user_echange_rtt="FALSE";} ;
		$sql1 = "UPDATE `conges_config` SET `conf_valeur` = '$config_user_echange_rtt' WHERE `conf_nom` = 'user_echange_rtt' ;";
		$result = SQL::query($sql1);

		if($config_affiche_bouton_calcul_nb_jours_pris) {$config_affiche_bouton_calcul_nb_jours_pris="TRUE";} else {$config_affiche_bouton_calcul_nb_jours_pris="FALSE";} ;
		...
		$result = SQL::query($sql1);
		if($config_rempli_auto_champ_nb_jours_pris) {$config_rempli_auto_champ_nb_jours_pris="TRUE";} else {$config_rempli_auto_champ_nb_jours_pris="FALSE";} ;
		...
		$result = SQL::query($sql1);
		if($config_verif_droits) {$config_verif_droits="TRUE";} else {$config_verif_droits="FALSE";} ;
