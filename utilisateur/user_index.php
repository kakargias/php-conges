...
if($_SESSION['config']['where_to_find_user_email']=="ldap"){ include CONFIG_PATH .'config_ldap.php';}
	...
	// if( $_SESSION['config']['auth'] && $_SESSION['config']['calendar'] )
		// $onglets['changer_mot_de_passe'] = _('calendar');		
	...
	header_menu('user',$onglet,$add_css);
	...
	// if ($onglet == $key) {$active = 'class=selected';}else{$active = '';}
		echo '<li class="'.($onglet == $key ? 'selected': '').'">
	...
	echo '<div id="tabs-1"  class="'.$onglet.'">';
