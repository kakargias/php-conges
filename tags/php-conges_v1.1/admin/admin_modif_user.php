...
*/
	...
	// TITRE
	if($u_login!="")
		echo "<H1>Modification utilisateur : $u_login .</H1>\n\n";
	elseif($u_login_to_update!="")
		echo "<H1>Modification utilisateur : $u_login_to_update .</H1>\n\n";
...
function modifier($u_login, $tab_checkbox_sem_imp, $tab_checkbox_sem_p, $DEBUG=FALSE)
{
	...
	echo "<td class=\"histo\">see_all</td>\n";
	...
	echo "<tr align=\"center\">\n";
	echo "<td class=\"histo\"></td><td class=\"histo\">nb jours / an </td><td class=\"histo\"></td><td class=\"histo\">solde</td><td class=\"histo\"></td>\n";
	echo "</tr>\n";
	foreach($tab_type_conges as $id_type_cong => $libelle)
	{
		...
		// jours / an
		echo "<td class=\"histo\">".$tab_user['conges'][$id_type_cong]['nb_an']."</td>\n";
		$text_jours_an="<input type=\"text\" name=\"tab_new_jours_an[$id_type_cong]\" size=\"5\" maxlength=\"5\" value=\"".$tab_user['conges'][$id_type_cong]['nb_an']."\">" ;
		echo "<td class=\"histo\">$text_jours_an</td>\n";
		// solde
		echo "<td class=\"histo\">".$tab_user['conges'][$id_type_cong]['solde']."</td>\n";
		$text_solde_jours="<input type=\"text\" name=\"tab_new_solde[$id_type_cong]\" size=\"5\" maxlength=\"5\" value=\"".$tab_user['conges'][$id_type_cong]['solde']."\">" ;
...
function commit_update($u_login_to_update, &$tab_new_user, &$tab_new_jours_an, &$tab_new_solde, $tab_checkbox_sem_imp, $tab_checkbox_sem_p, $DEBUG=FALSE)
{
	...
	{
		$valid=verif_saisie_decimal($tab_new_jours_an[$id_conges], $DEBUG);  //verif la bonne saisie du nombre décimal
		$valid=verif_saisie_decimal($tab_new_solde[$id_conges], $DEBUG);  //verif la bonne saisie du nombre décimal
	}
	...
	if($tab_new_user['login'] != $u_login_to_update)
	{
		...
		$result5 = requete_mysql($sql_upd_periode, $mysql_link, "commit_update", $DEBUG);
		...
		$result6 = requete_mysql($sql_upd_echange, $mysql_link, "commit_update", $DEBUG);
