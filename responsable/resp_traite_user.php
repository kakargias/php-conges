...
<?php 
	...
	if(isset($tab_checkbox_annule)) {
		annule_conges($user_login, $tab_checkbox_annule, $tab_text_annul, $mysql_link);
	}
	// si le traitement des demandes a été selectionée :
	elseif(isset($tab_radio_traite_demande)) {
		traite_demandes($user_login, $tab_radio_traite_demande, $tab_text_refus, $mysql_link);
	}
	// si un nouveau conges ou absence a été saisi pour un user :
	elseif($new_demande_conges==1) {
		new_conges($user_login, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $mysql_link);
	}
	else {
		affichage($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link);
	}
...
function affichage($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link)
{
	$PHP_SELF=$_SERVER['PHP_SELF']; ;
	$session=session_id();
...
function annule_conges($user_login, $tab_checkbox_annule, $tab_text_annul, $mysql_link) 
{
	$PHP_SELF=$_SERVER['PHP_SELF']; ;
	$session=session_id() ;
	...
	while($elem_tableau = each($tab_checkbox_annule))
	{
		...
		if($user_type=="conges") // on re-crédite les jours seulement pour des conges pris (pas pour les absences autres)
		{
			...
			//echo($sql2."<br>");
			$ReqLog2 = mysql_query($sql2, $mysql_link) or die("ERREUR : mysql_query : $sql1 :\n".mysql_error());
		}
...
function traite_demandes($user_login, $tab_radio_traite_demande, $tab_text_refus, $mysql_link) 
{
	$PHP_SELF=$_SERVER['PHP_SELF']; ;
	$session=session_id();
	...
	while($elem_tableau = each($tab_radio_traite_demande))
	{
		...
		else 
		{
			if(strcmp($value_traite, "REFUSE")==0) 
			{
				...
				$ReqLog3 = mysql_query($sql3, $mysql_link) or die("ERREUR : mysql_query : $sql3 :\n".mysql_error());
...
function new_conges($user_login, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $mysql_link)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$sessionsession_id();
	...
	if($valid==TRUE)
	{
		...
		/**********************************/
		$new_etat="ok";
		$result=insert_dans_periode($user_login, $new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $new_etat, $mysql_link);
