...
if($_SESSION['config']['where_to_find_user_email']=="ldap"){ include CONFIG_PATH .'config_ldap.php';}
	...
	if($count4==0)
	{
		echo '<div class="nouvelle_absence">';
		...
		echo "<div  style=\"width:70%\"><b>";
		echo "<a href=\"$PHP_SELF?session=$session&onglet=historique_autres_absences&year_affichage=$year_affichage_prec\"><span class=\"ui-icon ui-icon-circle-arrow-w\" style=\"float:left;\"></span></a>";
		echo "<center>&nbsp&nbsp&nbsp  $year_affichage &nbsp&nbsp&nbsp</center>";
		echo "<a href=\"$PHP_SELF?session=$session&onglet=historique_autres_absences&year_affichage=$year_affichage_suiv\"><span class=\"ui-icon ui-icon-circle-arrow-e\" style=\"float:right;\" ></span></a>";
		echo "</b></div><br><br>\n";
	...
	echo "</b></div><br><br>\n";
		// AFFICHAGE TABLEAU
		echo "<table id=\"histo_abs\" width=\"80%\">\n";
		echo "<thead>\n";
		echo "<tr>\n";
		echo "<th>\n";
		...
		while ($resultat4 = $ReqLog4->fetch_array())
		{
			...
			echo '<tr class="'.($i?'i':'p').'">';
				echo '<td class="histo"><span style=display:none>'.$resultat4["p_date_deb"].'</span>'.schars($sql_date_deb).' _ '.schars($demi_j_deb).'</td>';
				echo '<td class="histo"><span style=display:none>'.$resultat4["p_date_fin"].'</span>'.schars($sql_date_fin).' _ '.schars($demi_j_fin).'</td>' ;
