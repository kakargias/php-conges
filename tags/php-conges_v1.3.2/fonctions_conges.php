...
#$DEBUG=TRUE;
	...
	else
	{
		...
		$select_abs="SELECT p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours  FROM conges_periode WHERE p_num=$num_periode ";;
		...
		$sql_date_deb = $tab_date_deb[2]." / ".$tab_date_deb[1]." / ".$tab_date_deb[0]." (".$rec_abs["p_demi_jour_deb"].")" ;  // affiche : "23 / 01 / 2008 (am)"
		$tab_date_fin= explode("-", $rec_abs["p_date_fin"]);
		$sql_date_fin = $tab_date_fin[2]." / ".$tab_date_fin[1]." / ".$tab_date_fin[0]." (".$rec_abs["p_demi_jour_fin"].")" ;  // affiche : "23 / 01 / 2008 (am)"
