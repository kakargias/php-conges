...
//renvoi le total des congés pris sur l'année pour un utilisateur donné. Aucun enregistrement d'édition n'est effectué.
function get_year_edition_user($login, $DEBUG=FALSE)
{
    $today = getdate();
    $sql3 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_date_demande, p_date_traitement, ta_libelle ";
    $sql3=$sql3."FROM conges_periode AS a, conges_type_absence AS b ";
    $sql3=$sql3."WHERE (p_etat!='demande' AND p_etat!='valid') ";
    $sql3=$sql3."AND (p_login = '$login') ";
    $sql3=$sql3."AND (a.p_type=b.ta_id AND  ( (b.ta_type='conges') OR (b.ta_type='conges_exceptionnels') ) )";
    $sql3=$sql3."AND p_date_deb LIKE  '".$today['year']."%'";
    $sql3=$sql3."ORDER BY p_date_deb ASC ";
    $ReqLog3 = SQL::query($sql3);

    $tab=array();
    while ($resultat_ed = $ReqLog3->fetch_array())
    {
        //p_login   p_date_deb  p_demi_jour_deb p_date_fin  p_demi_jour_fin p_nb_jours  p_commentaire   p_type  p_etat  p_date_demande  p_date_traitement   ta_libelle
        $soustab=array();
        $soustab['p_login']           = $resultat_ed["p_login"];
        $soustab['p_date_deb']        = $resultat_ed["p_date_deb"];
        $soustab['p_demi_jour_deb']   = $resultat_ed["p_demi_jour_deb"];
        $soustab['p_date_fin']        = $resultat_ed["p_date_fin"];
        $soustab['p_demi_jour_fin']   = $resultat_ed["p_demi_jour_fin"];
        $soustab['p_nb_jours']        = $resultat_ed["p_nb_jours"];
        $soustab['p_commentaire']     = $resultat_ed["p_commentaire"];
        $soustab['p_type']            = $resultat_ed["p_type"];
        $soustab['p_etat']            = $resultat_ed["p_etat"];
        $soustab['p_date_demande']    = $resultat_ed["p_date_demande"];
        $soustab['p_date_traitement'] = $resultat_ed["p_date_traitement"];
        $soustab['ta_libelle']        = $resultat_ed["ta_libelle"];
        $soustab['p_date_deb']        = $resultat_ed["p_date_deb"];
...
// renvoit un tableau vide si pas d'edition pour le user
