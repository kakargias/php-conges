...
//function verif_saisie_decimal(&$nombre, $DEBUG)
...
// renvoit un tableau des types de conges
//function recup_tableau_types_conges($mysql_link)

// renvoit un tableau des types d'absence
//function recup_tableau_types_absence($mysql_link)

// renvoit un tableau de tableau contenant les infos des types de conges et absences
//function recup_tableau_tout_types_abs($mysql_link)

// renvoit le type d'absence (conges ou absence) d'une absence 
//function get_type_abs($_type_abs_id, $mysql_link)

// renvoit le libelle d'une absence (conges ou absence) d'une absence 
//function get_libelle_abs($_type_abs_id, $mysql_link)
...
//function affiche_tableau_bilan_conges_user($login, $mysql_link)

// renvoit un tableau de tableau contenant les informations du user
//function recup_infos_du_user($login, $mysql_link, $DEBUG=FALSE)
...
//
// Affichage d'un SELECT de dformulaire web à partir des valeurs du enum(...) d'un champ mysql (cf structure des tables)
...
//function affiche_select_from_mysql_enum_field($table, $column, $form_field, $default_value, $style)
	...
	$mois_calendrier_saisie_fin_suiv=0; $year_calendrier_saisie_fin_suiv=0; 
		...
		printf("<form action=\"$PHP_SELF?session=$session\" method=\"POST\">\n" ) ;
			...
			echo "<td>\n";
				...
				$tab_type_absence=recup_tableau_types_absence($mysql_link);
...
function alerte_mail($login_expediteur, $destinataire, $objet, $DEBUG=FALSE)
{
	require_once($_SESSION['config']['php_conges_phpmailer_include_path']."/phpmailer/class.phpmailer.php");	// ajout de la classe phpmailer
...
function insert_dans_periode($login, $date_deb, $demi_jour_deb, $date_fin, $demi_jour_fin, $nb_jours, $commentaire, $id_type_abs, $etat, $mysql_link, $DEBUG=FALSE)
{
	...
	$sql2 = "INSERT INTO conges_periode 
			...
			p_type='$id_type_abs', p_etat='$etat', p_num='$num_new_demande' " ;
...
//	$tab['php_conges_include_path']=$config_php_conges_document_root."/INCLUDE.PHP" ;
	...
	//  recup des variables de config.php
	...
	$req = mysql_query($sql) or die ("ERREUR : mysql_query --> $sql ".mysql_error());
	...
	//  recup des variables de config.php
	if(isset($config_semaine_bgcolor)) {$tab['semaine_bgcolor']=$config_semaine_bgcolor ;}
	if(isset($config_week_end_bgcolor)) {$tab['week_end_bgcolor']=$config_week_end_bgcolor ;}
	if(isset($config_temps_partiel_bgcolor)) {$tab['temps_partiel_bgcolor']=$config_temps_partiel_bgcolor ;}
	if(isset($config_conges_bgcolor)) {$tab['conges_bgcolor']=$config_conges_bgcolor ;}
	if(isset($config_demande_conges_bgcolor)) {$tab['demande_conges_bgcolor']=$config_demande_conges_bgcolor ;}
	if(isset($config_absence_autre_bgcolor)) {$tab['absence_autre_bgcolor']=$config_absence_autre_bgcolor ;}
	
	if(isset($config_mail_sujet_new_demande)) {$tab['mail_sujet_new_demande']=$config_mail_sujet_new_demande ;}
	if(isset($config_mail_contenu_new_demande)) {$tab['mail_contenu_new_demande']=$config_mail_contenu_new_demande ;}
	if(isset($config_mail_sujet_valid_conges)) {$tab['mail_sujet_valid_conges']=$config_mail_sujet_valid_conges ;}
	if(isset($config_mail_contenu_valid_conges)) {$tab['mail_contenu_valid_conges']=$config_mail_contenu_valid_conges ;}
	if(isset($config_mail_sujet_refus_conges)) {$tab['mail_sujet_refus_conges']=$config_mail_sujet_refus_conges ;}
	if(isset($config_mail_contenu_refus_conges)) {$tab['mail_contenu_refus_conges']=$config_mail_contenu_refus_conges ;}
	if(isset($config_mail_sujet_annul_conges)) {$tab['mail_sujet_annul_conges']=$config_mail_sujet_annul_conges ;}
	if(isset($config_mail_contenu_annul_conges)) {$tab['mail_contenu_annul_conges']=$config_mail_contenu_annul_conges ;}
...
{
	$tab=array();
	$sql_cong="SELECT ta_id, ta_libelle FROM conges_type_absence WHERE ta_type='conges' ";
...
{
	$tab=array();
	$sql_abs="SELECT ta_id, ta_libelle FROM conges_type_absence WHERE ta_type='absence' ";
...
function get_libelle_abs($_type_abs_id, $mysql_link)
{
	...
	$ReqLog_abs = mysql_query($sql_abs, $mysql_link) or die("ERREUR : mysql_query : <br>\n".$sql_abs." --> ".mysql_error());
	if($resultat_abs = mysql_fetch_array($ReqLog_abs))
...
function recup_tableau_conges_for_user($login, $mysql_link, $DEBUG=FALSE)
{
	$sql_bilan = "SELECT su_abs_id, su_nb_an, su_solde FROM conges_solde_user where su_login = '$login' ORDER BY su_abs_id ASC";
	...
	{
		$tab=array();
		$sql_id=affiche_decimal($resultat_bilan["su_abs_id"]);
...
function affiche_tableau_bilan_conges_user($login, $mysql_link, $DEBUG=FALSE)
{
	...
	if($DEBUG==TRUE) { echo"tab_type_cong =<br>\n"; print_r($tab_type_cong); echo "<br>\n"; }
	...
	echo "<tr align=\"center\"><td class=\"titre\">quotité</td>" ;
	foreach($tab_cong_user as $id => $val)
		echo "<td class=\"titre\">".$tab_type_cong[$id]."/ AN</td><td class=\"titre\">SOLDE ".$tab_type_cong[$id]."</td>" ;
	echo "</tr>\n";
...
function recup_infos_all_users($mysql_link, $DEBUG=FALSE)
{
	...
	$sql = "SELECT u_login, u_nom, u_prenom, u_is_resp, u_resp_login, u_is_admin, u_see_all, u_passwd, u_quotite, u_email FROM conges_users " .
			"WHERE u_login!='conges' AND u_login!='admin' ORDER BY u_nom, u_prenom";
	...
	while ($resultat = mysql_fetch_array($ReqLog)) 
	{
		...
		$tab_user['quotite']=$resultat["u_quotite"];
		$tab_user['email']=$resultat["u_email"];
		$tab_user['conges']=recup_tableau_conges_for_user($sql_login, $mysql_link, $DEBUG);
		
		$tab[$sql_login] = $tab_user ;
	}
...
function recup_infos_all_users_du_resp($login, $mysql_link, $DEBUG=FALSE)
{
	...
	$sql = "SELECT u_login, u_nom, u_prenom, u_is_resp, u_resp_login, u_is_admin, u_see_all, u_passwd, u_quotite, u_email FROM conges_users";
	if($list_all_users_du_resp!="")  //si la liste n'est pas vide ( serait le cas si n'est responsable d'aucun users ni groupe)
		$sql = $sql." WHERE u_login IN ($list_all_users_du_resp) ";
	$sql = $sql." ORDER BY u_nom, u_prenom";
	
	$ReqLog = requete_mysql($sql, $mysql_link, "recup_infos_all_users_du_resp", $DEBUG) ;
...
function affiche_select_from_mysql_enum_field($table, $column, $form_field, $default_value, $style)
{
   ...
   if ($style  == "")
   {
      printf("<SELECT NAME=$form_field>\n");
   }
   else
   {
      printf("<SELECT NAME=$form_field STYLE=\"$style\">\n");
   }

   $mysql_link = connexion_mysql();
   $req_enum = "DESCRIBE $table $column";
   $res_enum = mysql_query($req_enum,$mysql_link) or die("erreur".mysql_error());
