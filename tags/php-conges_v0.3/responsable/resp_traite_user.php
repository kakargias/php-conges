...
function affichage($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin) {
	...
	printf("<tr align=\"center\"><td class=\"titre\">Quotité</td><td class=\"titre\">nb jours/an</td><td class=\"titre\">SOLDE</td></tr>\n");
	$ReqLog1 = mysql_query($sql1, $link) or die("ERREUR : mysql_query : ".$sql1." --> ".mysql_error());
	...
	else
	{
		...
		while ($resultat3 = mysql_fetch_array($ReqLog3)) {
				$etat=$resultat3[5];
				if(($etat!="annulé")&&($etat!="absence-annulée")) {
					$casecocher1=sprintf("<input type=\"checkbox\" name=\"tab_checkbox_annule[%s]\" value=\"%s-%s-%sANNULE\">", $resultat3["p_num"], $resultat3["p_login"], $resultat3["p_etat"], affiche_decimal($resultat3["p_nb_jours"]) );
				}
				else {
					$casecocher1="";
				}
...
function annule_conges($user_login) {
	...
	while($elem_tableau = each($tab_checkbox_annule))
	{
		...
		$user_etat=$champs[2];
		echo("<br><br>conges numero :".$numero." --- User_login : ".$user_login." --- nb de jours : ".$user_nb_jours_pris_float." --- etat : ".$user_etat." ---> ANNULER <br>");
		...
		if($user_etat=="pris") // on recrédite les jours seulement pour des conges pris (pas pour les absences autres)
		{
