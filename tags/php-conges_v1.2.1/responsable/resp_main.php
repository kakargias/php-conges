...
echo "</head>\n";
	...
	{
		echo "<td class=\"titre\"> $libelle/ ".$_SESSION['lang']['divers_an_maj']."</td><td class=\"titre\">".$_SESSION['lang']['divers_solde_maj']." $libelle</td>";
	}
	...
	if(count($tab_all_users)==0) // si le tableau est vide (resp sans user !!) on affiche une alerte !
		echo "<tr align=\"center\"><td class=\"histo\" colspan=\"9\">".$_SESSION['lang']['resp_etat_aucun_user']."</td></tr>\n" ;
	else
	{
		...
		foreach($tab_all_users_2 as $current_login_2 => $tab_current_user_2)
		{
			if(in_array($current_login_2, $tab_all_users_2)==FALSE) // si le user n'est pas déjà dans le tableau précédent (deja affiché)
			{
				...
				if($compteur==1)  // alors on affiche une ligne de titre
					echo "<tr align=\"center\"><td class=\"histo\" colspan=\"9\"><i>".$_SESSION['lang']['resp_etat_users_titre_double_valid']."</i></td></tr>\n";
