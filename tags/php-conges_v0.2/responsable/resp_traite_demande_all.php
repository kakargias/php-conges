...
function saisie() {
	...
	printf("<tr align=\"center\"><td>nom</td><td>prenom</td><td>Quotité</td><td>Debut</td><td>Fin</td><td>NB_Jours_Pris</td><td>Commentaire</td><td>Solde</td><td>Accepter</td><td>Refuser</td><td>Attente</td></tr>\n");
	$tab_bt_radio=array();
	while ($resultat1 = mysql_fetch_array($ReqLog1)) {
			...
			$boutonradio1=sprintf("<input type=\"radio\" name=\"tab_bt_radio[%s]\" value=\"%s\">", $resultat1["p_num"], $resultat1["p_login"]."-".$resultat1["u_nb_jours_reste"]."-".$resultat1["p_nb_jours"]."-OK" );
			$boutonradio2=sprintf("<input type=\"radio\" name=\"tab_bt_radio[%s]\" value=\"%s\">", $resultat1["p_num"], $resultat1["p_login"]."-".$resultat1["u_nb_jours_reste"]."-".$resultat1["p_nb_jours"]."-not_OK" );
			$boutonradio3=sprintf("<input type=\"radio\" name=\"tab_bt_radio[%s]\" value=\"%s\" checked>", $resultat1["p_num"], $resultat1["p_login"]."-".$resultat1["u_nb_jours_reste"]."-".$resultat1["p_nb_jours"]."-RIEN" );
...
function traite_demande() {
	...
	while($elem_tableau = each($tab_bt_radio))
	{
		...
		$user_login=$champs[0];
		$user_nb_jours_reste=$champs[1];
		$user_nb_jours_pris=$champs[2];
		$etat=$champs[3];
		...
		if(strcmp($etat, "OK")==0)
		{
			...
			$sql2 = "UPDATE conges_users SET u_nb_jours_reste=$user_nb_jours_reste-$user_nb_jours_pris WHERE u_login='$user_login' " ;
			//echo($sql2."<br>");
			/* On valide l'UPDATE dans la table ! */
