...
function affichage($onglet, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $tri_date, $mysql_link, $DEBUG=FALSE)
{
	...
	echo "<tr align=\"center\">\n";
		...
		if($onglet!="historique_conges")
			echo "<td class=\"onglet\" width=\"170\"><a href=\"$PHP_SELF?session=$session&onglet=historique_conges\" class=\"bouton-onglet\"> Historique des congés </a></td>\n";
		else
			echo "<td class=\"current-onglet\" width=\"170\"><a href=\"$PHP_SELF?session=$session&onglet=historique_conges\" class=\"bouton-current-onglet\"> Historique des congés </a></td>\n";
	...
	{
		echo "<td colspan=$nb_colonnes>\n";
		echo "<H3>Echange jour rtt,temps partiel / jour travaillé :</H3>\n\n";
	...
	{
		echo "<td colspan=$nb_colonnes>\n";
		echo "<h3>Historique des congés :</h3>\n";
	...
	{
		echo "<td colspan=$nb_colonnes>\n";
		echo "<h3>Historique des absences pour mission, formation, etc ... :</h3>\n";
	...
	if($valid==TRUE)
	{
		...
		if($result==TRUE)
		{
			...
			if($_SESSION['config']['mail_new_demande_alerte_resp']==TRUE)
				alerte_mail($_SESSION['userlogin'], ":responsable:", "new_demande", $DEBUG);
		}
		else
...
function affichage_demandes_en_cours($tri_date, $onglet, $mysql_link, $DEBUG=FALSE)
{
	...
	$sql3 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_motif_refus, p_num, ta_libelle
...
function affichage_historique_conges($tri_date, $onglet, $mysql_link, $DEBUG=FALSE)
{
	...
	$sql2 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_motif_refus, ta_libelle
			 FROM conges_periode as a, conges_type_absence as b 
			WHERE a.p_login = '".$_SESSION['userlogin']."' 
			AND (a.p_type=b.ta_id) 
			AND (b.ta_type='conges')
...
function affichage_historique_absences($tri_date, $onglet, $mysql_link, $DEBUG=FALSE)
{
	...
	// Récupération des informations
	$sql4 = "SELECT p_login, p_date_deb, p_demi_jour_deb, p_date_fin, p_demi_jour_fin, p_nb_jours, p_commentaire, p_type, p_etat, p_motif_refus, p_num, ta_libelle
			FROM conges_periode as a, conges_type_absence as b 
			WHERE a.p_login = '".$_SESSION['userlogin']."' 
			AND (a.p_type=b.ta_id) 
