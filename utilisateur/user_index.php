	...
	if($valid==TRUE)
	{
		...
		if($result==TRUE)
		{
			...
			if($_SESSION['config']['mail_new_demande_alerte_resp']==TRUE)
				alerte_mail($_SESSION['userlogin'], ":responsable:", "new_demande", $mysql_link, $DEBUG);
		}
		else
...
function affichage_demandes_en_cours($tri_date, $onglet, $mysql_link, $DEBUG=FALSE)
{
	...
	else
	{
		...
		while ($resultat3 = mysql_fetch_array($ReqLog3)) 
		{
			...
			//on ne peut pas modifier une demande qui a déja étét validé une foir (si on utilise la double validation)
			if($sql_p_etat=="valid")
				$user_modif_demande="&nbsp;";
			else
