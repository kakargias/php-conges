...
defined( '_PHP_CONGES' ) or die( 'Restricted access' );
	...
	$jourferie = $_SESSION["tab_j_feries"];
...
$(function() {
	...
	$( "#common" ).datepicker(
		...
		onSelect: function(dtxt) {alert(testingdate(dtxt))} 
		});
	$( "#exchange" ).datepicker(
		{
		showWeek: true,
...
function testingdate(dateText) {
var d = new Date();
d.setDate(parseInt(dateText));
n=d.getDay();
var type = disabledays(d)[1];
if (type == 'j') {$("#radioj").attr("checked",true);$("#labelj").attr("class","ui-button ui-widget ui-state-default ui-button-text-only ui-corner-right ui-state-active");$("#labelj").attr("aria-pressed",true);}
if (type == 'm') {$("#radiom").attr("checked",true);$("#labelm").attr("class","ui-button ui-widget ui-state-default ui-button-text-only ui-corner-right ui-state-active");$("#labelm").attr("aria-pressed",true);}
if (type == 'pm') {$("#radioam").attr("checked",true);$("#labelam").attr("class","ui-button ui-widget ui-state-default ui-button-text-only ui-corner-right ui-state-active");$("#labelam").attr("aria-pressed",true);}
return type;
...
var unavailableDates = <?php echo test_date_rtt ($_SESSION['userlogin']);?>;
var ferie = <?php echo json_encode($jourferie);?>;
function disabledays(date) {
	// alert(date);Mon Mar 26 2012 00:00:00 GMT+0200
	var day = date.getDay();
	// var num_semaine = DefSemaineNum( date.getFullYear(), strpad((date.getMonth() + 1)),strpad(date.getDate()));
	var num_semaine = $.datepicker.iso8601Week(new Date(date.getFullYear(), strpad((date.getMonth() + 1)) - 1, strpad(date.getDate())));
	jours = new Array('di', 'lu', 'ma', 'me', 'je', 've', 'sa'); 
	var da = jours[day];
	if (num_semaine % 2 == 0) {var sem = 'p'} else {var sem = 'imp'}
	dmy =  'sem_'+sem+'_'+da;
    

    if ((jQuery.inArray(dmy+'_pm', unavailableDates) >= 0 ) && (jQuery.inArray(dmy+'_am', unavailableDates) >= 0 ) )
	{return [true, "j", "ARTT"]} 
	else if(jQuery.inArray(dmy+'_am', unavailableDates) >= 0  )  {return [true, "am", "ARTT"]} 
	else if (jQuery.inArray(dmy+'_pm', unavailableDates) >= 0 )  {return [true, "pm", "ARTT"]} 
    else { return [false,dmy,""];}
	}
...
function enabledays(date) {
	...
	var da = jours[day];
	if (num_semaine % 2 == 0) {var sem = 'p'} else {var sem = 'imp'}
	var spi =  'sem_'+sem+'_'+da;
    

    if ((jQuery.inArray(spi+'_pm', unavailableDates) >= 0 ) && (jQuery.inArray(spi+'_am', unavailableDates) >= 0 ) )
	{return [false, "", "ARTT"]} 
	else if(jQuery.inArray(spi+'_am', unavailableDates) >= 0 )  {return [false, "", "ARTT"]} 
	else if (jQuery.inArray(spi+'_pm', unavailableDates) >= 0 )  {return [false, "", "ARTT"]} 
	else if (day == 0 || day == 6 ||(jQuery.inArray(dmy, ferie) >= 0 ))  {return [false, "", "WE"]} 
    else { return [true,"",""];}
	 // if (jQuery.inArray(dmy, ferie) >= 0 ) {return [false,"",""];}
	}
	...
	echo '<div class="nouvelle_absence_1">';
	echo '<div  style="float:left">';
	echo '<div id="common"></div>';
	echo '<div  class="radioline">';
	echo '<label id="labelm" for="radiom">Matin</label><input type="radio" id="radiom" name="moment_absence_ordinaire" value="a" >';
	echo '<label id="labelam" for="radioam">AM</label><input id="radioam" type="radio" name="moment_absence_ordinaire" value="p" >';
	echo '<label id="labelj" for="radioj">Jour</label><input id="radioj" type="radio" name="moment_absence_ordinaire" value="j"  >';
	echo '</div>';
	echo '</div>';
	echo '<div style="float:left">';
	echo '<div  id="exchange"></div>';
	echo '<div  class="radioline">';
	echo '<label for="radio1m">Matin</label><input id="radio1m" type="radio" name="moment_absence_souhaitee" value="a">';
	echo '<label for="radio1am">AM</label><input id="radio1am" type="radio" name="moment_absence_souhaitee" value="a">';
	echo '<label for="radio1j">Jour</label><input id="radio1j" type="radio" name="moment_absence_souhaitee" value="a" >';
	echo '</div>';
	echo '</div>';
	echo '<div class=clear></div>';
	// echo test_date_rtt ($_SESSION['userlogin']);
