...
function lance_maj($lang, $installed_version, $etape, $DEBUG=FALSE)
{
	...
	elseif($etape==4)
	{
		...
		{
			if($DEBUG==FALSE)
				echo "<META HTTP-EQUIV=REFRESH CONTENT=\"0; URL=$PHP_SELF?etape=5&version=1.2&lang=$lang\">";
			else
				echo "<a href=\"$PHP_SELF?etape=5&version=1.2&lang=$lang\">".$_SESSION['lang']['install_etape']." 4  OK</a><br>\n";
		}
	...
	elseif($etape==5)
	{
		...
		if( (test_config_file($DEBUG)==TRUE) || (test_old_config_file($DEBUG)==TRUE) )
		{
			...
			echo "<br><a href=\"$PHP_SELF?etape=5&version=1.2&lang=$lang\">".$_SESSION['lang']['install_reload_page']." ....</a><br>\n";
		}
		else
		{	
