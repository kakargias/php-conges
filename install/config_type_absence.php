...
function affichage($tab_new_values, $mysql_link, $session, $DEBUG=FALSE)
{
	...
	while ($data = mysql_fetch_array($ReqLog1)) 
	{
		...
		if($old_type != $ta_type)
		{
			...
			if($ta_type=="conges")
			{
				$legend=$_SESSION['lang']['divers_conges_maj_1'] ;
				$comment=$_SESSION['lang']['config_abs_comment_conges'] ;
			}
			else
			{
				$legend=$_SESSION['lang']['divers_absences_maj_1'] ;
				$comment=$_SESSION['lang']['config_abs_comment_absences'] ;
			}
			echo "<br>\n";
	...
	echo "    <td class=\"histo\">\n";
	affiche_select_from_mysql_enum_field("conges_type_absence", "ta_type", "tab_new_values[type]", $new_type, "", $mysql_link, $DEBUG) ;
	echo "    </td></tr>\n";
	echo "    </table>\n";
