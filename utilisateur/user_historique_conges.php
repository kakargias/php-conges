...
if($_SESSION['config']['where_to_find_user_email']=="ldap"){ include CONFIG_PATH .'config_ldap.php';}
	...
	{	
		...
		echo "<a href=\"$PHP_SELF?session=$session&onglet=historique_conges&year_affichage=$year_affichage_prec\"><span class=\"ui-icon ui-icon-circle-arrow-w\" style=\"float:left;\"></span></a>";
		echo '<center>&nbsp&nbsp&nbsp  '.schars($year_affichage).' &nbsp&nbsp&nbsp</center>';
		echo '<a href="'.schars($PHP_SELF).'?session='.schars($session).'&onglet=historique_conges&year_affichage='.schars($year_affichage_suiv).'"><span class="ui-icon ui-icon-circle-arrow-e" style="float:right;" ></span></a>';
		echo "</b></div><br><br>\n";
	...
	echo "</b></div><br><br>\n";
		...
		while ($resultat2 = $ReqLog2->fetch_array())
		{
			...
			echo '<tr class="'.($i?'i':'p').'">';
				echo '<td class="histo"><span style=display:none>'.$resultat2["p_date_deb"].'</span>'.schars($sql_p_date_deb).' _ '.schars($demi_j_deb).'</td>';
				echo '<td class="histo"><span style=display:none>'.$resultat2["p_date_fin"].'</span>'.schars($sql_p_date_fin).' _ '.schars($demi_j_fin).'</td>' ;
