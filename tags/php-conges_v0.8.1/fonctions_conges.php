...
//function affiche_cellule_jour_cal_saisie($j_timestamp, $year, $mois, $jour, $td_second_class, $result)
...
//	$mois_name=date("F", $first_jour_mois_timestamp);
	...
	for($i=$first_jour_mois_rang; $i<8; $i++) {
		...
		affiche_cellule_jour_cal_saisie($j_timestamp, $year, $mois, $j, $td_second_class, "new_debut");
	}
	echo "</tr>\n";
	...
	for($i=8-$first_jour_mois_rang+1; $i<15-$first_jour_mois_rang+1; $i++) {
		...
		affiche_cellule_jour_cal_saisie($j_timestamp, $year, $mois, $i, $td_second_class, "new_debut");
	}
	echo "</tr>\n";
...
//	$mois_name=date("F", $first_jour_mois_timestamp);
	...
	for($i=$first_jour_mois_rang; $i<8; $i++) {
		...
		affiche_cellule_jour_cal_saisie($j_timestamp, $year, $mois, $j, $td_second_class, "new_fin");
	}
	echo "</tr>\n";
	...
	for($i=8-$first_jour_mois_rang+1; $i<15-$first_jour_mois_rang+1; $i++) {
		...
		affiche_cellule_jour_cal_saisie($j_timestamp, $year, $mois, $i, $td_second_class, "new_fin");
	}
	echo "</tr>\n";
...
global $config_samedi_travail, $config_dimanche_travail ;
	...
	echo "<td>\n";
		...
		{
			$sa_am="<input type=\"checkbox\" name=\"tab_checkbox_sem_imp[sem_imp_sa_am]\" value=\"Y\" $checked_option_sem_imp_sa_am>";
			$sa_pm="<input type=\"checkbox\" name=\"tab_checkbox_sem_imp[sem_imp_sa_pm]\" value=\"Y\" $checked_option_sem_imp_sa_pm>";
		}
		if($config_dimanche_travail==TRUE)
		{
			$di_am="<input type=\"checkbox\" name=\"tab_checkbox_sem_imp[sem_imp_di_am]\" value=\"Y\" $checked_option_sem_imp_di_am>";
			$di_pm="<input type=\"checkbox\" name=\"tab_checkbox_sem_imp[sem_imp_di_pm]\" value=\"Y\" $checked_option_sem_imp_di_pm>";
		}
		...
		echo "</tr>\n";
		echo "<tr align=\"center\">\n";
			echo "<td class=\"histo\">matin</td><td class=\"histo\">$lu_am</td><td class=\"histo\">$ma_am</td><td class=\"histo\">$me_am</td><td class=\"histo\">$je_am</td><td class=\"histo\">$ve_am</td>\n";
			if($config_samedi_travail==TRUE)
				echo "<td class=\"histo\">$sa_am</td>\n";
			if($config_dimanche_travail==TRUE)
				echo "<td class=\"histo\">$di_am</td>\n";
		echo "</tr>\n";
		echo "<tr align=\"center\">\n";
			echo "<td class=\"histo\">apres-midi</td><td class=\"histo\">$lu_pm</td><td class=\"histo\">$ma_pm</td><td class=\"histo\">$me_pm</td><td class=\"histo\">$je_pm</td><td class=\"histo\">$ve_pm</td>\n";
			if($config_samedi_travail==TRUE)
				echo "<td class=\"histo\">$sa_pm</td>\n";
			if($config_dimanche_travail==TRUE)
				echo "<td class=\"histo\">$di_pm</td>\n";
		echo "</tr>\n";
		echo "</table>\n";
	...
	echo " <td>\n";
		...
		$sa_am="<input type=\"checkbox\" name=\"tab_checkbox_sem_p[sem_p_sa_am]\" value=\"Y\" $checked_option_sem_p_sa_am>";
		$sa_pm="<input type=\"checkbox\" name=\"tab_checkbox_sem_p[sem_p_sa_pm]\" value=\"Y\" $checked_option_sem_p_sa_pm>";
		$di_am="<input type=\"checkbox\" name=\"tab_checkbox_sem_p[sem_p_di_am]\" value=\"Y\" $checked_option_sem_p_di_am>";
		$di_pm="<input type=\"checkbox\" name=\"tab_checkbox_sem_p[sem_p_di_pm]\" value=\"Y\" $checked_option_sem_p_di_pm>";
...
//				if( ($date_j>=$tab_grille_user[$key]["date_debut_grille"]) && ($date_j<=$tab_grille_user[$key]["date_fin_grille"]) ) // date_jour comprise entre date_deb_grille et date_fin grille
				...
				{
//					$val_matin=$tab_grille_user[$key][$key_artt_matin];
//					$val_aprem=$tab_grille_user[$key][$key_artt_aprem];
...
/*function date_fr_old($format, $timestamp) 
{ 
	...
	//$strDate = preg_replace("/(\d)(st|th|nd|rd)/", "\${1}", $strDate); //st, th, nd ou rd qui est précédé d'un chiffre 
...
// affichage de la cellule correspondant au jour dans les calendrier de saisie (demande de conges, etc ...)
function affiche_cellule_jour_cal_saisie($j_timestamp, $year, $mois, $j, $td_second_class, $result)
{
global $session, $session_username;
global $link;
	...
	recup_infos_artt_du_jour($session_username, $j_timestamp, $val_matin, $val_aprem);
	...
	echo "<td  class=\"cal-saisie $td_second_class $class_am $class_pm\">$j<input type=\"radio\" name=\"$result\" value=\"$year-$mois-$j\"></td>";
...
function get_list_users_des_groupes_du_resp($resp_login)
{
	...
	{
		$sql="SELECT DISTINCT(gu_login) FROM conges_groupe_users WHERE gu_groupename IN ($list_groups) ORDER BY gu_login ";
...
function get_list_groupes_du_resp($resp_login)
{
	...
	$sql="SELECT gr_groupename FROM conges_groupe_resp WHERE gr_login='$resp_login' ORDER BY gr_groupename";
	$ReqLog1 = mysql_query($sql, $link) or die("ERREUR : get_list_groupes_du_resp() : ".mysql_error());
	while ($resultat1 = mysql_fetch_array($ReqLog1)) 
	{
		$current_group=$resultat1["gr_groupename"];
		if($list_group=="")
