...
if($_SESSION['config']['where_to_find_user_email']=="ldap"){ include CONFIG_PATH .'config_ldap.php';}
	...
	echo '<h1>'. _('user_etat_demandes') .' :</h1><br/>';
		// AFFICHAGE TABLEAU
		echo '<table id="dmd_in" width="80%">';
		echo '<thead>';
			echo '<tr>';
				echo '<th>';
		...
		while ($resultat3 = $ReqLog3->fetch_array()) {
			...
			echo '<tr>';
				echo '<td><span style=display:none>'.$resultat3["p_date_deb"].'</span>'.schars($sql_p_date_deb).' _ '.schars($demi_j_deb).'</td>';
				echo '<td><span style=display:none>'.$resultat3["p_date_fin"].'</span>'.schars($sql_p_date_fin).' _ '.schars($demi_j_fin).'</td>' ;
				echo '<td>'.schars($sql_p_type).'</td>' ;
				echo '<td>'.affiche_decimal($sql_p_nb_jours).'</td>' ;
				echo '<td>'.schars($sql_p_commentaire).'</td>' ;
				if( !$_SESSION['config']['interdit_modif_demande'] ) {
					echo '<td><span class="ui-icon ui-icon-pencil" style="float:left;"></span>'.($user_modif_demande).'</td>' ;
				}
				echo '<td ><span class="ui-icon ui-icon-trash" style="float:left;"></span>'.($user_suppr_demande).'</td>'."\n" ;
				
				if( $_SESSION['config']['affiche_date_traitement'] ) {
					if($sql_p_date_demande == NULL)
						echo '<td >'. _('divers_demande') .' : '.$sql_p_date_demande.'<br>'. _('divers_traitement') .' : '.$sql_p_date_traitement.'</td>';
					else
						echo '<td >'. _('divers_demande') .' : '.$sql_p_date_demande.'<br>'. _('divers_traitement') .' : pas traité</td>';
				}
			echo '</tr>';
			$i = !$i;
		}
