...
function alerte_mail($login_expediteur, $destinataire, $objet, $DEBUG=FALSE)
{
	...
	init_tab_corps_mail($tab_corps_mail, $DEBUG=FALSE);
...
// construit le tableau des sujets et corps des messages à partir du contenu du fichier de config
function init_tab_corps_mail(&$tab_corps_mail, $DEBUG=FALSE)
{
...
//	$tab['php_conges_include_path']=$config_php_conges_document_root."/INCLUDE.PHP" ;
	...
	// on verifie si on est dans le répertoire "install" ou ailleurs ...
	if(strstr($script_path, "install")!=FALSE)
...
function affiche_tableau_bilan_conges_user($login, $mysql_link, $DEBUG=FALSE)
{
	...
	foreach($tab_cong_user as $id => $val)
		echo "<td class=\"titre\">".$tab_type_cong[$id]."/ ".$_SESSION['lang']['divers_annee_maj']."</td><td class=\"titre\">".$_SESSION['lang']['divers_solde_maj']." ".$tab_type_cong[$id]."</td>" ;
	echo "</tr>\n";
