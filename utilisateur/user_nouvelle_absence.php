...
defined( '_PHP_CONGES' ) or die( 'Restricted access' );
	...
	if( $new_demande_conges == 1 && $_SESSION['config']['user_saisie_demande'] ) {
		...
		echo '<h1 style="color:white;">'. _('divers_nouvelle_absence') ."  ".$user_login.' :</h1>';
		...
		saisie_nouveau_conges_datepicker($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet,  $DEBUG=FALSE) ;
...
function new_demande($new_debut, $new_demi_jour_deb, $new_fin, $new_demi_jour_fin, $new_nb_jours, $new_comment, $new_type, $DEBUG=FALSE)
{
	...
	if( $valid ) {
		...
		if ( $periode_num != 0 ) {
			echo '<div style="width:30%" id="dmd_ok" class="ui-state-highlight ui-corner-all" style="padding: 0 .7em;"><p>';
			echo '<span class="ui-icon ui-icon-circle-check" style="float:left;"></span>'.schars( _('form_modif_ok') ).' !<br><br>'."\n";
			echo '</p></div>';
