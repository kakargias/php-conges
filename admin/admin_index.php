...
<?php 
	...
	{
		$tab_new_user['password1']= getpost_variable("new_password1") ;
		$tab_new_user['password2']= getpost_variable("new_password2") ;
	}
	...
	$tab_new_user['new_jour']= getpost_variable("new_jour") ;
	$tab_new_user['new_mois']= getpost_variable("new_mois") ;
	$tab_new_user['new_year']= getpost_variable("new_year") ;
	...
	echo " <a href=\"javascript:void(0);\" onClick=\"javascript:window.close();\"><img src=\"../img/exit.png\" width=\"22\" height=\"22\" border=\"0\" title=\"Fermeture du mode Administrateur\" alt=\"Fermeture du mode Administrateur\"></a> Fermer\n";
	echo "</td>\n";

	/* bouton config php_conges  */
	if($_SESSION['config']['affiche_bouton_config_pour_admin']==TRUE)
	{
		echo "<td width=\"110\" align=\"left\" valign=\"top\">\n";
		echo " <a href=\"javascript:void(0);\" onClick=\"javascript:OpenPopUp('../install/configure.php?session=$session','mapage',800,600);\"><img src=\"../img/tux_config_32x32.png\" width=\"32\" height=\"32\" border=\"0\" title=\"Configuration de php_conges\" alt=\"Configuration de php_conges\"></a> Configuration\n";
		echo "</td>\n";
	}
	else
	{
		/* cellule vide  */
		echo "<td width=\"110\" valign=\"middle\">&nbsp;</td>\n";
	}
	
	/* bouton config types absence php_conges  */
	if($_SESSION['config']['affiche_bouton_config_absence_pour_admin']==TRUE)
	{
		echo "<td width=\"130\" align=\"left\" valign=\"top\">\n";
		echo " <a href=\"javascript:void(0);\" onClick=\"javascript:OpenPopUp('../install/config_type_absence.php?session=$session','mapage',800,600);\"><img src=\"../img/tux_config_32x32.png\" width=\"32\" height=\"32\" border=\"0\" title=\"Configuration des types d'absence gérées par php_conges\" alt=\"Configuration des types d'absence gérées par php_conges\"></a> Config Absences\n";
		echo "</td>\n";
	}
	else
	{
		/* cellule vide  */
		echo "<td width=\"130\" valign=\"middle\">&nbsp;</td>\n";
	}
	...
	if($saisie_user=="ok") {
		ajout_user($tab_new_user, $tab_checkbox_sem_imp, $tab_checkbox_sem_p, $tab_new_jours_an, $tab_new_solde, $mysql_link, $DEBUG);
	}
	elseif($saisie_group=="ok") {
		ajout_groupe($new_group_name, $new_group_libelle, $mysql_link, $DEBUG);
	}
	...
	{
		modif_group_responsables($choix_group, $checkbox_group_resp, $mysql_link, $DEBUG);
	}
	elseif($change_responsable_group=="ok")
	{
		modif_resp_groupes($choix_resp, $checkbox_resp_group, $mysql_link, $DEBUG);
	}
	else {
		/* affichage normal */
		affichage($onglet, $new_group_name, $new_group_libelle, $choix_group, $choix_user, $choix_resp, $tab_new_user, $tab_new_jours_an, $tab_new_solde, $mysql_link, $DEBUG); 
	}
...
function affichage($onglet, $new_group_name, $new_group_libelle, $choix_group, $choix_user, $choix_resp, &$tab_new_user, &$tab_new_jours_an, &$tab_new_solde, $mysql_link, $DEBUG=FALSE)
{
	...
	echo "<tr align=\"center\">\n";
		...
		if($onglet!="ajout-user")
			echo "<td class=\"onglet\" width=\"170\"><a href=\"$PHP_SELF?session=$session&onglet=ajout-user\" class=\"bouton-onglet\"> Ajout d'un Utilisateur </a></td>\n";
		else
			echo "<td class=\"current-onglet\" width=\"170\"><a href=\"$PHP_SELF?session=$session&onglet=ajout-user\" class=\"bouton-current-onglet\"> Ajout d'un Utilisateur </a></td>\n";
...
function ajout_user(&$tab_new_user, $tab_checkbox_sem_imp, $tab_checkbox_sem_p, &$tab_new_jours_an, &$tab_new_solde, $mysql_link, $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
...
//		|| strlen($tab_new_user['solde_jours'])==0 
		|| $tab_new_user['quotite']>100) ) )
	{
		echo "<H3> ATTENTION : certain champs saisis ne sont pas valides ...... </H3>\n"  ;
		// affichage des param :
		...
		if ($num_verif!=0)
		{
			echo "<H3> ATTENTION : login déjà utilisé, veuillez en changer ...... </H3>\n"  ;
			echo "<form action=\"$PHP_SELF?session=$session\" method=\"POST\">\n"  ;
			echo "<input type=\"hidden\" name=\"new_login\" value=\"".$tab_new_user['login']."\">\n";
			echo "<input type=\"hidden\" name=\"new_nom\" value=\"".$tab_new_user['nom']."\">\n";
			echo "<input type=\"hidden\" name=\"new_prenom\" value=\"".$tab_new_user['prenom']."\">\n";
		...
		{
			echo "<H3> ATTENTION : adresse mail éronnée ...... </H3>\n" ;
			echo "<form action=\"$PHP_SELF?session=$session\" method=\"POST\">\n" ;
			echo "<input type=\"hidden\" name=\"new_login\" value=\"".$tab_new_user['login']."\">\n";
			echo "<input type=\"hidden\" name=\"new_nom\" value=\"".$tab_new_user['nom']."\">\n";
			echo "<input type=\"hidden\" name=\"new_prenom\" value=\"".$tab_new_user['prenom']."\">\n";
...
function affiche_gestion_utilisateurs($mysql_link, $DEBUG=FALSE)
{
	...
	// AFFICHAGE TABLEAU
	echo "<h3>Etat des Utilisateurs :</h3>\n";
	...
	{
		echo "<td class=\"titre\">$libelle / an</td>\n";
	...
	echo "<td class=\"titre\">see_all</td>\n";
	...
	foreach($tab_info_users as $current_login => $tab_current_infos)
	{
		...
		$admin_modif_user="<a href=\"admin_modif_user.php?session=$session&u_login=$current_login\">Modifier</a>" ;
		$admin_suppr_user="<a href=\"admin_suppr_user.php?session=$session&u_login=$current_login\">Supprimer</a>" ;
		$admin_chg_pwd_user="<a href=\"admin_chg_pwd_user.php?session=$session&u_login=$current_login\">Password</a>" ;
		...
		{
			echo "<td class=\"histo\">".$tab_conges[$id_conges]['nb_an']."</td>\n";
...
function affiche_formulaire_ajout_user(&$tab_new_user, &$tab_new_jours_an, &$tab_new_solde, $mysql_link, $DEBUG=FALSE)
{
	...
	// TITRE
	echo "<H3><u>Nouvel Utilisateur :</u></H3>\n\n";
	...
	echo "<td class=\"histo\">voir_tous</td>\n";
	if($_SESSION['config']['where_to_find_user_email']=="dbconges")
...
//	echo "<table cellpadding=\"2\" class=\"tablo\" width=\"80%\">\n";
	...
	echo "<td class=\"histo\">nb / an</td>\n";
	echo "<td class=\"histo\">solde</td>\n";
	echo "</tr>\n";
	...
	echo "<input type=\"submit\" value=\"Valider Nouvel Utilisateur\">\n";
	echo "</form>\n" ;
...
function ajout_groupe($new_group_name, $new_group_libelle, $mysql_link, $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
...
function modif_group_responsables($choix_group, &$checkbox_group_resp, $mysql_link, $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
...
function modif_resp_groupes($choix_resp, &$checkbox_resp_group, $mysql_link, $DEBUG=FALSE)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
