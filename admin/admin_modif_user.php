...
<?php
	...
	if(isset($_POST['new_jour'])) { $tab_new_user['jour']=$_POST['new_jour']; }
	if(isset($_POST['new_mois'])) { $tab_new_user['mois']=$_POST['new_mois']; }
	if(isset($_POST['new_year'])) { $tab_new_user['year']=$_POST['new_year']; }
	/*************************************/
	...
	if(isset($u_login)) {
		modifier($u_login, $tab_checkbox_sem_imp, $tab_checkbox_sem_p);
	}
	else {
		if(isset($u_login_to_update)) {
			commit_update($u_login_to_update, $tab_new_user, $tab_checkbox_sem_imp, $tab_checkbox_sem_p);
		}
		else {
...
function modifier($u_login, $tab_checkbox_sem_imp, $tab_checkbox_sem_p) {
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
	...
	$sql1 = "SELECT u_login, u_nom, u_prenom, u_nb_jours_an, u_solde_jours, u_nb_rtt_an, u_solde_rtt, u_is_resp, u_resp_login, u_is_admin, u_quotite, u_email FROM conges_users WHERE u_login = '$u_login' " ;
	// AFFICHAGE TABLEAU
	...
	echo "<td class=\"histo\">is_admin</td>\n";
	...
	while ($resultat1 = mysql_fetch_array($ReqLog1)) 
	{
		...
		$sql_is_admin=$resultat1["u_is_admin"];
	...
	saisie_jours_absence_temps_partiel($u_login,$mysql_link);
...
function commit_update($u_login_to_update, &$tab_new_user, $tab_checkbox_sem_imp, $tab_checkbox_sem_p)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
	...
	echo "$u_login_to_update---".$tab_new_user['nom']."---".$tab_new_user['prenom']."---".$tab_new_user['quotite']."---".$tab_new_user['nb_j_an']."---".$tab_new_user['solde_jours']."---".$tab_new_user['rtt_an']."---".$tab_new_user['solde_rtt']."---".$tab_new_user['is_resp']."---".$tab_new_user['resp_login']."---".$tab_new_user['is_admin']."---".$tab_new_user['email']."---".$tab_new_user['login']."<br>\n";
	...
	$valid=verif_saisie_decimal($tab_new_user['nb_j_an']);        //verif la bonne saisie du nombre décimal
	...
	$sql1 = "UPDATE conges_users  
		SET	u_nom='".$tab_new_user['nom']."', 
			u_prenom='".$tab_new_user['prenom']."',
			u_nb_jours_an='".$tab_new_user['nb_j_an']."', 
			u_solde_jours='".$tab_new_user['solde_jours']."', 
			u_nb_rtt_an='".$tab_new_user['rtt_an']."', 
			u_solde_rtt='".$tab_new_user['solde_rtt']."', 
	...
	$result1 = mysql_query($sql1, $mysql_link) or die("ERREUR : commit_update() : $sql1 <br>\n".mysql_error());
	if($result1==FALSE)
		$result==FALSE;
	
	/*************************************/
	/* Mise a jour de la table artt si besoin :   */
	$tab_grille_rtt_actuelle = get_current_grille_rtt($u_login_to_update, $mysql_link);
	...
	else
	{
		...
		$result_grille = mysql_query($sql_grille, $mysql_link) or die("ERREUR : commit_update() : $sql_grille <br>\n".mysql_error());
		...
		if($count_grille==0) // si pas de grille modifiée aujourd'hui : on update la date de fin de la derniere grille
		{
			...
			//echo "sql2 = $sql2<br>\n";
			$result2 = mysql_query($sql2, $mysql_link) or die("ERREUR : commit_update() : $sql2 <br>\n".mysql_error());
			if($result2==FALSE)
				$result==FALSE;
		}
		else  // si une grille modifiée aujourd'hui : on delete cette grille
		{
			...
			$result_suppr_grille = mysql_query($sql_suppr_grille, $mysql_link) or die("ERREUR : commit_update() : $sql_suppr_grille <br>\n".mysql_error());
		...
		if( ($list_columns!="") && ($list_valeurs!="") )
		{
			...
			//echo "sql3 = $sql3<br>\n";
			$result3 = mysql_query($sql3, $mysql_link) or die("ERREUR : commit_update() : \n$sql3\n".mysql_error());
			if($result3==FALSE)
				$result==FALSE;
		}
	}
	...
	if($tab_new_user['login'] != $u_login_to_update)
	{
		...
		$result4 = mysql_query($sql_upd_artt, $mysql_link) or die("ERREUR : commit_update() : ".mysql_error());
		if($result4==FALSE)
			$result==FALSE;
		...
		$result5 = mysql_query($sql_upd_periode, $mysql_link) or die("ERREUR : commit_update() : ".mysql_error());
		if($result5==FALSE)
			$result==FALSE;
		...
		$result6 = mysql_query($sql_upd_echange, $mysql_link) or die("ERREUR : commit_update() : ".mysql_error());
		if($result6==FALSE)
			$result==FALSE;
	}
...
function get_current_grille_rtt($u_login_to_update, $mysql_link)
{
	$tab_grille=array();
	
	$sql1 = "SELECT * FROM conges_artt WHERE a_login='$u_login_to_update' AND a_date_fin_grille='9999-12-31' "  ;
	$ReqLog1 = mysql_query($sql1, $mysql_link) or die("ERREUR : get_current_grille_rtt() : ".mysql_error());
	while ($resultat1 = mysql_fetch_array($ReqLog1)) {
