...
//function  affiche_calendrier_saisie_date_debut($user_login, $year, $mois, $mysql_link) {

// affichage du calendrier avec les case à cocher, du mois de fin du congés 
//function  affiche_calendrier_saisie_date_fin($user_login, $year, $mois, $mysql_link) {

// affichage du calendrier du mois avec les case à cocher sur les jour d'absence 
//function  affiche_calendrier_saisie_jour_absence($user_login, $year, $mois, $mysql_link) {

// affichage du calendrier du mois avec les case à cocher sur les jour de présence
//function  affiche_calendrier_saisie_jour_presence($user_login, $year, $mois, $mysql_link)
...
//function saisie_nouveau_conges($login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link)

//affiche le formulaire d'échange d'un jour de rtt-temps partiel / jour travaillé
//function saisie_echange_rtt($year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link)
...
//function recup_infos_artt_du_jour($sql_login, $j_timestamp, &$val_matin, &$val_aprem, $mysql_link)

// recup des infos ARTT ou Temps Partiel depuis un tableau préalablement construit :
// attention : les param $val_matin et $val_aprem sont passées par référence (avec &) car on change leur valeur
//function recup_infos_artt_du_jour_from_tab($sql_login, $j_timestamp, &$val_matin, &$val_aprem, $tab_rtt_echange, $tab_rtt_planifiees, $mysql_link)
...
//function alerte_mail($nom_login, $destinataire, $objet)

// recuperation du mail d'un user
//function find_email_adress_for_user($login)

// construit le tableau des sujets et corps des messages à partir du contenu du fichier de config
// le tableau (vide ) est passé en parametre
//function init_tab_corps_mail(&$tab_corps_mail)
...
//function affiche_cellule_jour_cal_saisie($login, $j_timestamp, $td_second_class, $result, $mysql_link)

// recup de la liste des users des groupes dont $resp_login est responsable 
// renvoit une liste de login entre quotes et séparés par des virgules
//function get_list_users_des_groupes_du_resp($resp_login, $mysql_link)
...
//function get_tab_resp_du_user($user_login, &$tab_resp, $mysql_link)
...
//function init_tab_jours_feries($mysql_link)
...
//	$mois_name=date("F", $first_jour_mois_timestamp);
	...
	for($i=$first_jour_mois_rang; $i<8; $i++) {
		...
		affiche_cellule_jour_cal_saisie($user_login, $j_timestamp, $td_second_class, "new_debut", $mysql_link);
	}
	echo "</tr>\n";
...
//	$mois_name=date("F", $first_jour_mois_timestamp);
	...
	for($i=$first_jour_mois_rang; $i<8; $i++) {
		...
		affiche_cellule_jour_cal_saisie($user_login, $j_timestamp, $td_second_class, "new_fin", $mysql_link);
	}
	echo "</tr>\n";
...
function  affiche_calendrier_saisie_jour_absence($user_login, $year, $mois, $mysql_link) 
{
	...
	for($i=$first_jour_mois_rang; $i<8; $i++) {
		...
		{
			$bgcolor=$_SESSION['config']['semaine_bgcolor'];
			recup_infos_artt_du_jour($user_login, $j_timestamp, $val_matin, $val_aprem, $mysql_link);
...
function saisie_nouveau_conges($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link)
{
					...
					echo "<fieldset class=\"cal_saisie\">\n";
						...
						echo "<tr align=\"center\">\n";
							echo "<td>\n";
								...
								affiche_calendrier_saisie_date_debut($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $mysql_link);  
							echo "</td>\n";
					...
					echo "<fieldset class=\"cal_saisie\">\n";
						...
						echo "<tr align=\"center\">\n";
							echo "<td>\n";
								...
								affiche_calendrier_saisie_date_fin($user_login, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $mysql_link);  
							echo "</td>\n";
							// cellule 2 : boutons radio matin ou après midi
							echo "<td align=\"left\">\n";
			...
			echo "<td>\n";
				...
				echo "<td valign=\"top\">\n";
					...
					if($_SESSION['config']['affiche_bouton_calcul_nb_jours_pris']==TRUE)
					{
						printf("<tr><td colspan=\"2\">\n");
							...
							echo "<input type=\"button\" onclick=\"compter_jours(new_debut, new_fin, login_user, new_demi_jour_deb, new_demi_jour_fin);return false;\" value=\"Compter les jours\">\n";
						printf("</td></tr>\n");
					}
				...
				echo "<td align=\"left\" valign=\"top\">\n";
					    ...
					    (($_SESSION['config']['user_saisie_demande']==FALSE)&&($user_login!=$_SESSION['userlogin'])) )
					{
						...
						if($_SESSION['config']['rtt_comme_conges']==TRUE) // si on gère les rtt comme des congés
...
//affiche le formulaire d'échange d'un jour de rtt-temps partiel / jour travaillé
function saisie_echange_rtt($user_login, $year_calendrier_saisie_debut, $mois_calendrier_saisie_debut, $year_calendrier_saisie_fin, $mois_calendrier_saisie_fin, $onglet, $mysql_link)
{
	$PHP_SELF=$_SERVER['PHP_SELF'];
	$session=session_id();
...
function recup_infos_artt_du_jour($sql_login, $j_timestamp, &$val_matin, &$val_aprem, $mysql_link)
{
	...
	{  
		...
		$res_echange_rtt = mysql_query($sql_echange_rtt, $mysql_link) or die("ERREUR : recup_infos_artt_du_jour() <br>\n$sql_echange_rtt<br>\n".mysql_error());
		...
		else
		{
			...
			$res_artt = mysql_query($sql_artt, $mysql_link) or die("ERREUR : recup_infos_artt_du_jour() <br>\n $sql_artt<br>\n".mysql_error());
...
function verif_saisie_decimal(&$nombre)
{
	...
	if ( !ereg( "([0-9]+)([\.\,]*[0-9]{1,2})*", $nombre) ) {
		echo "<br>ERREUR : mauvaise saisie : <b>le nombre saisi est invalide</b><br>\n";
		$verif=FALSE ;
	}
	else {
...
// parametre 3=objet du message (cf fichier de config pour les diff valeurs possibles)
function alerte_mail($login_expediteur, $destinataire, $objet)
{
	if(is_dir('INCLUDE.PHP'))
		$path_to_racine="";
	else
		$path_to_racine="../";
	
	require_once($path_to_racine."/INCLUDE.PHP/phpmailer/class.phpmailer.php");	// ajout de la classe phpmailer
	//require_once($path_to_racine."INCLUDE.PHP/fonction.php");			// pour la cnx à la base de données
	//include($path_to_racine."config.php");						// pour les paramètres généraux...
	...
	$mail->SetLanguage("fr", $path_to_racine."INCLUDE.PHP/phpmailer/language/");
	...
	if($destinataire==":responsable:")  // c'est une message au responsable 
	{
		...
		$tab_resp=array();
		get_tab_resp_du_user($login_expediteur, $tab_resp, $mysql_link);
	...
	// construction du tableau des sujets et corps des messages 
	$tab_corps_mail=array();
	init_tab_corps_mail($tab_corps_mail);
	//print_r($tab_corps_mail); echo "<br><br>\n";
	
	// construction du corps du mail
	$mail->Subject  =  $tab_corps_mail[$objet]["sujet"];
	$mail->Body     =  $mail->FromName." ".$tab_corps_mail[$objet]["contenu"];
	if( $DEBUG )
	{
		echo "SUBJECT = ".$tab_corps_mail[$objet]["sujet"]."<br>\n";
		echo "CONTENU = ".$tab_corps_mail[$objet]["contenu"]."<br>\n";
	}
...
//		include('config_ldap.php');
	...
	elseif($_SESSION['config']['where_to_find_user_email']=="dbconges") // recherche du mail du user dans la base db_conges
	{
		...
		$res = mysql_query($req, $mysql_link) or die('erreur '.mysql_error());
...
function init_tab_corps_mail(&$tab_corps_mail)
{
	$tab_corps_mail["new_demande"] =array("sujet" => $_SESSION['config']['mail_sujet_new_demande'] , "contenu" => $_SESSION['config']['mail_contenu_new_demande']);
	$tab_corps_mail["valid_conges"]=array("sujet" => $_SESSION['config']['mail_sujet_valid_conges'], "contenu" => $_SESSION['config']['mail_contenu_valid_conges']);
	$tab_corps_mail["refus_conges"]=array("sujet" => $_SESSION['config']['mail_sujet_refus_conges'], "contenu" => $_SESSION['config']['mail_contenu_refus_conges']);
	$tab_corps_mail["annul_conges"]=array("sujet" => $_SESSION['config']['mail_sujet_annul_conges'], "contenu" => $_SESSION['config']['mail_contenu_annul_conges']);
...
function affiche_cellule_jour_cal_saisie($login, $j_timestamp, $td_second_class, $result, $mysql_link)
{
	...
	recup_infos_artt_du_jour($login, $j_timestamp, $val_matin, $val_aprem, $mysql_link);
	...
	// Si le client est sous IE, on affiche pas les jours de rtt (car IE ne gère pas les appels stadart de classe de feuille e style)
...
function get_list_users_des_groupes_du_resp($resp_login, $mysql_link)
{
	...
	$list_groups=get_list_groupes_du_resp($resp_login, $mysql_link);
	if($list_groups!="") // si $resp_login est responsable d'au moins un groupe
	{
		...
		$ReqLog1 = mysql_query($sql, $mysql_link) or die("ERREUR : get_list_users_des_groupes_du_resp() : ".mysql_error());
		while ($resultat1 = mysql_fetch_array($ReqLog1)) 
		{
...
function get_list_groupes_du_resp($resp_login, $mysql_link)
{
	...
	$ReqLog1 = mysql_query($sql, $mysql_link) or die("ERREUR : get_list_groupes_du_resp() : ".mysql_error());
	while ($resultat1 = mysql_fetch_array($ReqLog1)) 
	{
...
function get_list_users_des_groupes_du_user($user_login, $mysql_link)
{
	...
	$list_groups=get_list_groupes_du_user($user_login, $mysql_link);
	if($list_groups!="") // si $user_login est membre d'au moins un groupe
	{
		$sql="SELECT DISTINCT(gu_login) FROM conges_groupe_users WHERE gu_gid IN ($list_groups) ORDER BY gu_login ";
		$ReqLog1 = mysql_query($sql, $mysql_link) or die("ERREUR : get_list_users_des_groupes_du_user() : <br>\n".mysql_error());
		while ($resultat1 = mysql_fetch_array($ReqLog1)) 
		{
			$current_login=$resultat1["gu_login"];
			if($list_users=="")
...
function get_list_groupes_du_user($user_login, $mysql_link)
{
	...
	$ReqLog1 = mysql_query($sql, $mysql_link) or die("ERREUR : get_list_groupes_du_user() : <br>\n".mysql_error());
	while ($resultat1 = mysql_fetch_array($ReqLog1)) 
	{
...
function get_tab_resp_du_user($user_login, &$tab_resp, $mysql_link)
{
	...
	else
	{
		...
		$res = mysql_query($req, $mysql_link) or die("ERREUR : get_tab_resp_du_user() :<br>\n $req<br>\n ".mysql_error());
		$rec = mysql_fetch_array($res);
		...
		if($_SESSION['config']['gestion_groupes']==TRUE)
		{
			...
			foreach($tab_gid as $gid)
			{
				...
				$ReqLog1 = mysql_query($sql, $mysql_link) or die("ERREUR : get_tab_resp_du_user() :<br>\n $sql<br>\n ".mysql_error());
				while ($resultat1 = mysql_fetch_array($ReqLog1)) 
				{
...
function init_tab_jours_feries($mysql_link)
{
	...
	$res_select = mysql_query($sql_select, $mysql_link) or die("ERREUR : init_tab_jours_feries() <br>\n$sql_select<br>\n".mysql_error());
...
function init_config_tab()
{
	...
	//  config.php
	$tab['mysql_serveur']=$mysql_serveur ;
	$tab['mysql_user']=$mysql_user ;
	$tab['mysql_pass']=$mysql_pass ;
	$tab['mysql_database']=$mysql_database  ;
	
	$tab['php_conges_version']=$config_php_conges_version;
	$tab['url_site_web_php_conges']=$config_url_site_web_php_conges;

	$tab['img_login']=$config_img_login;
	$tab['texte_img_login']=$config_texte_img_login ;
	$tab['lien_img_login']=$config_lien_img_login ;

	$tab['titre_page_accueil']=$config_titre_page_accueil ;
	$tab['titre_calendrier']=$config_titre_calendrier ;
	$tab['titre_user_index']=$config_titre_user_index ;
	$tab['titre_resp_index']=$config_titre_resp_index ;
	$tab['titre_admin_index']=$config_titre_admin_index ;

	$tab['URL_ACCUEIL_CONGES']=$URL_ACCUEIL_CONGES ;

	$tab['auth']=$config_auth ;

	$tab['how_to_connect_user']=$config_how_to_connect_user ;

	$tab['export_users_from_ldap']=$config_export_users_from_ldap ;

	$tab['responsable_virtuel']=$config_responsable_virtuel ;

	$tab['user_saisie_demande']=$config_user_saisie_demande ;

	$tab['mail_new_demande_alerte_resp']=$config_mail_new_demande_alerte_resp ;
	$tab['mail_valid_conges_alerte_user']=$config_mail_valid_conges_alerte_user ;
	$tab['mail_refus_conges_alerte_user']=$config_mail_refus_conges_alerte_user ;
	$tab['mail_annul_conges_alerte_user']=$config_mail_annul_conges_alerte_user ;

	$tab['serveur_smtp']=$config_serveur_smtp ;
	$tab['where_to_find_user_email']=$config_where_to_find_user_email ;

	$tab['samedi_travail']=$config_samedi_travail ;
	$tab['dimanche_travail']=$config_dimanche_travail ;

	$tab['gestion_groupes']=$config_gestion_groupes ;
	
	$tab['editions_papier']=$config_editions_papier ;

	$tab['texte_haut_edition_papier']=$config_texte_haut_edition_papier ;

	$tab['texte_bas_edition_papier']=$config_texte_bas_edition_papier ;

	$tab['user_affiche_calendrier']=$config_user_affiche_calendrier ;

	$tab['resp_affiche_calendrier']=$config_resp_affiche_calendrier ;

	$tab['affiche_groupe_in_calendrier']=$config_affiche_groupe_in_calendrier ;

	$tab['affiche_bouton_calcul_nb_jours_pris']=$config_affiche_bouton_calcul_nb_jours_pris ;
	$tab['rempli_auto_champ_nb_jours_pris']=$config_rempli_auto_champ_nb_jours_pris ;

	$tab['user_saisie_mission']=$config_user_saisie_mission ;

	$tab['resp_saisie_mission']=$config_resp_saisie_mission ;

	$tab['rtt_comme_conges']=$config_rtt_comme_conges ;

	$tab['user_echange_rtt']=$config_user_echange_rtt ;

	$tab['user_ch_passwd']=$config_user_ch_passwd ;

	$tab['resp_vertical_menu']=$config_resp_vertical_menu ;

	$tab['affiche_reference_edition_papier']=$config_affiche_reference_edition_papier ;

	$tab['admin_see_all']=$config_admin_see_all ;

	$tab['admin_change_passwd']=$config_admin_change_passwd ;

	$tab['duree_session']=$duree_session ;

	$tab['stylesheet_file']=$config_stylesheet_file ;
	$tab['bgcolor']=$config_bgcolor ;
	$tab['bgimage']=$config_bgimage ;
	$tab['light_grey_bgcolor']=$config_light_grey_bgcolor ;
	$tab['semaine_bgcolor']=$config_semaine_bgcolor ;
	$tab['week_end_bgcolor']=$config_week_end_bgcolor ;
	$tab['temps_partiel_bgcolor']=$config_temps_partiel_bgcolor ;
	$tab['conges_bgcolor']=$config_conges_bgcolor ;
	$tab['demande_conges_bgcolor']=$config_demande_conges_bgcolor ;
	$tab['absence_autre_bgcolor']=$config_absence_autre_bgcolor ;

	$tab['verif_droits']=$config_verif_droits ;

	$tab['mail_sujet_new_demande']=$config_mail_sujet_new_demande ;
	$tab['mail_contenu_new_demande']=$config_mail_contenu_new_demande ;
	$tab['mail_sujet_valid_conges']=$config_mail_sujet_valid_conges ;
	$tab['mail_contenu_valid_conges']=$config_mail_contenu_valid_conges ;
	$tab['mail_sujet_refus_conges']=$config_mail_sujet_refus_conges ;
	$tab['mail_contenu_refus_conges']=$config_mail_contenu_refus_conges ;
	$tab['mail_sujet_annul_conges']=$config_mail_sujet_annul_conges ;
	$tab['mail_contenu_annul_conges']=$config_mail_contenu_annul_conges ;
