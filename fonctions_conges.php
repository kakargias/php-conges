...
global $config_user_echange_rtt ;
	...
	//$tab_rtt_planifiees  //tableau indexé dont la clé est le login du user
					// il contient pour chaque clé : un tableau ($tab_user_rtt) qui contient lui même 
...
function alerte_resp_mail($nom_login)
{
	...
	$mail->Subject  =  "APPLI CONGES - Demande de congés";
	$mail->Body     =  $mail->FromName." a solicité une demande de congés dans l'application.
...
{
	$tab_rtt_planifiees=array();  //tableau indexé dont la clé est le login du user
					// il contient pour chaque clé : un tableau ($tab_user_rtt) qui contient lui même 
					// les infos pour le matin et l'après midi ('Y' si rtt, 'N' sinon) sur 2 semaines 
					// ( du sem_imp_lu_am au sem_p_ve_pm )
