...
//$DEBUG==TRUE;
	...
	if( (count($tab_all_users_du_resp)!=0) || (count($tab_all_users_du_grand_resp)!=0) )
	{
		...
		affichage_cloture_user_par_user($tab_type_conges, $tab_all_users_du_resp, $tab_all_users_du_grand_resp,  $DEBUG);
		echo "<br>\n";
		...
		foreach($tab_type_conges as $id_conges => $libelle)
		{
			...
			if($_SESSION['config']['autorise_reliquats_exercice']==TRUE)
			{
				...
				if($user_solde_actuel>0)
				{
					...
					$ReqLog_reliquat = requete_mysql($sql_reliquat,  "cloture_current_year_for_login", $DEBUG) ;
				}
				else
				...
				$ReqLog_solde = requete_mysql($sql_solde,  "cloture_current_year_for_login", $DEBUG) ;
			}
			else
			{
			...
			$ReqLog2 = requete_mysql($sql2,  "cloture_current_year_for_login", $DEBUG) ;
...
function update_appli_num_exercice( $DEBUG=FALSE)
{
	...
	$ReqLog_verif = requete_mysql($sql_verif,  "update_appli_num_exercice", $DEBUG) ;
	...
	if($ReqLog_verif->num_rows == 0)
	{
		...
		$ReqLog_update = requete_mysql($sql_update,  "update_appli_num_exercice", $DEBUG) ;
